...
begin                                         //      DaylyBalance
  ...
  DecodeDate(m_ReportDate, l_Year, l_Month, l_Day);
  m_ReportSvDate := m_ReportDate;
  PH_ReportName := cDateTimeR.GetNameMonth(l_Month)+ ' ' + IntToStr(l_Year) + ' года';
  ...
  for KindEn := 0 to 3 do begin
    ...
    if not IsBitInMask(bEnergMask, KindEn) then        // зависят отчекбоксов на форме (могут быть все 4)
      continue;
    ...
    if IsUsePokNow then begin  // текущие данные
      for i := 0 to 4 do begin
      // получение данных  1-й уровень
    ...
    end else begin  // архивные данные
...
//  TRepPower.Edit12.Text:=s;
  ...
  s:=PH_AbonentAddress;
  FDir:=FDir + 'Показания счетчиков электроэнергии ' + s + '.xls';

  m_DB.GetHomeBallanceReport(SList, XLSPoint, WB, Date2);  // выполнение запроса и занесение данных
