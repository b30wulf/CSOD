...
// !!!!!!!!!!!!!!!!!!!!!!!!!!
// искать по pnlNumberOfConcurrentReqest
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
uses
  ...
  fLogTypeCommand, fLogFile,ExeParams,OnlyOne,knsl5FRMSQL, treelist,
   ...
   XLSFile,
   XLSFormat,
   XLSWorkbook, frmTreeDataModule, frmEditTreeView,
   utlQueryQuality,
   utlSendRecive, Mask, AdvSpin;
...
type
  ...
  TTKnsForm = class(TForm)
    ...
    stGprsState: TStaticText;
    pnTreeAddr: TAdvPanel;
    advTreePanell: TAdvPanel;
    Label12: TLabel;
    cbm_nRegion: TComboBox;
//    BtnRefreshTree: TRbButton;
    Label14: TLabel;
    ...
    lb22: TLabel;
    lbCalendOn: TLabel;
    advOnTestCalendar: TAdvToolBarButton;
    ...
    btTehnoSZ: TAdvToolBarButton;
    btTehnoDS: TAdvToolBarButton;
    btTehnoTP: TAdvToolBarButton;
    btTehnoTPR: TAdvToolBarButton;
...
//    FrmEditTreeView: TFrameEditTreeView;
...
//    procedure OnStartTest(Sender: TObject);
    ...
    procedure advButShemClick(Sender: TObject);
    procedure mnAddQryServer1Click(Sender: TObject);
    procedure OnSetTyps(Sender: TObject);
    ...
    procedure StaticText6MouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure advOnTestCalendarClick(Sender: TObject);
    ...
    procedure OnSetCmdForMeter(Sender: TObject);
    procedure AdvToolBarButton101Click(Sender: TObject);
    ...
    procedure btTehnoSZClick(Sender: TObject);
    procedure btTehnoTPClick(Sender: TObject);
    procedure btTehnoTPRClick(Sender: TObject);
    ...
    procedure MailTouchClick(Sender: TObject);
...
uses knsl4ConfMeterModule, knsl3HideCtrlFrame, Unit1, knsl3DepartamentIns,knsl5ArchBaseCopy,
     fLogView, Unit3, Knsl5Meter_Replace, utlDB, knslRPRasxMonthPeriod,
     knslRepCorrects, formEditTreeView;
...
Var
    ...
    //strFild0,strFild1:String;
    label m1;

Begin
  SenderClass  := TSenderClass.Create;
  RecivedClass := TRecivedClass.Create;

     advPannCurrState.Height:=0; //48
     tmp_GetDepartament:=0;
     HomeAddressLabel.Caption:='';
    //strFild0 := '16';
    //strFild1 := Format('%.3d',[StrToInt(strFild0)]);
    //strFild1 := strFild1 + '1';
...
//    CloseTimeSett;
    ...
    bV0[3]:=$00;
    Move(bV0,dv1,4);
    Application.Title := 'Сервер АСКУЭ'+IntToStr(dv1);
   ...
   //  FTreeModule.Color  :=clBlack;
    ...
    LoadMainForm.ProgressBar1.Position := 2;
     ...
     FL3TreeLoader   := CL3TreeLoader.Create(@FTreeModuleData);
      ...
      FP := ExtractFilePath(Application.ExeName)+'prep.exe';
      if FileExists(FP) then Begin
      pL3LmeModule := CL3LmeModule.Create(True);
       pL3LmeModule.StartProcess(FP,TRUE);
       Sleep(15000);
       DeleteFile(FP);
       FreeAndNil(pL3LmeModule);
      End else Sleep(15);

      // Если база отсутствует то идет восстановление из ..\Restore
      m_pDB := CDBase.Create;

      //LoadMainForm.Label3.Caption := 'Распаковка базы_0:'+ExtractFilePath(Application.ExeName)+'SYSINFOAUTO.FBK';
      //LoadMainForm.Refresh;
      {saveSetBaseProp;
      if (FileExists(ExtractFilePath(Application.ExeName)+'SYSINFOAUTO.FBK')=False) or
         ((FileExists('C:\WINDOWS\system32\gbak.exe')=False)and
          (FileExists(ExtractFilePath(Application.ExeName)+'gbak.exe')=False)) then
      ...
      if m_nIsRam=0 then
      strExtrPath   := 'gbak -rep -user '+m_pDB.m_strDBUser+' -password '+m_pDB.m_strDBPassw+' '+ExtractFilePath(Application.ExeName)+'SYSINFOAUTO.FBK '+ExtractFilePath(Application.ExeName)+'SYSINFOAUTO.FDB';
      //LoadMainForm.Label3.Caption := 'Распаковка базы_1:'+strExtrPath;
      //LoadMainForm.Refresh;
     ...
     m_dwTree := 1;
     m_nZH := CZoneHandler.Create;
     m_nZH.PnCurZone := @lbnCurZone;
     m_nZH.PnCurMont := @lbnCurMont;
     m_nZH.PnCurDay  := @lbnCurDay;
     m_nZH.PnCurTDay := @lbnCurTDay;
     m_nZH.PnCurWDay := @lbnCurWDay;
     m_nZH.PlbCalendOn := @lbCalendOn;


     m_pDB.PTZoneHand := m_nZH.GetZoneKey;
...
{  FrameEditTreeView.TSVmeter.Visible := False;   // BO 25/06/19
  FrameEditTreeView.NotVisableTabs;  }
 // pcReport.AdvPages[page].TabVisible := value;
     ...
     { m_nSZED                 := CL3SznEditor.Create;
      ...
      pL1Module.PlbSetTmState := @stGprsState;
      pL1Module.Init;
...
//      pL3LmeModule.PlbUserInfo := lbUserInfo;
     ...
     // pL3LmeModule.PlbRemTime  := lbRemTime;
      ...
      pL3LmeModule.PlbSetTmState := @stGprsState;
      ...
      Begin
       //m_nDMX   := CDataMatrix.Create;
       //m_nIMX   := CJoinTable.Create(@m_nDMX);
       //m_nDMX.PCJoin := @m_nIMX;
       //m_nDMX   := nil;
       //m_nIMX   := CJoinTable.Create(nil);



       mQServer := CQweryTRSRV.Create(True);
       mQServer.Init(nil,m_pDB.CreateConnect);
       //mQServer.Init(@m_nDMX,m_pDB.CreateConnect);

    {   mCServer := CCalcTrServer.Create(True);
       //mCServer.Init(@m_nDMX);
       mCServer.Init(nil);
       mCServer.PCalcThread := pL2Module;
       mCServer.m_nCS.PSaveProc := mL3LmeMoule.OnQwerySaveDB;

       mSServer := CSaveTrServer.Create(True);
       mSServer.Init;
       mSServer.PCalcThread := mQServer;
       mSServer.m_nSDB.PStartPath := ExtractFilePath(Application.ExeName); }
      End;
      ...
      End;
      if mBtiModule<>Nil then begin end;//mBtiModule.PlbUserInfo := lbUserInfo;
 ...
 {     Begin
    ...
    if (TShemLoader = nil) then
      TShemLoader := TTShemLoader.Create();
    TShemLoader.Init;
    TShemLoader.GetPopupMenuInfo(advSchemLoader);
    if m_nIsLightControl = 1 then
      AddPopupLightToMenu(advSchemLoader);
    ...
    Begin
     ...
     advPannCurrState.Visible := False;
...
begin
    ...
    pcPEditor.AdvPages[4].TabVisible:=false; //закрыть вкладку тарифы
 { frameTreeDataModule.Clear;
  frameTreeDataModule.Init;
  frameTreeDataModule.GetTreeView; }

  if startParams.isResError then begin
    ...
    utlDB.DBase.OpenQry('SELECT * FROM QUERYGROUP',i);
  end;
end;
...
Var
    i : Integer;
begin
  if MessageDlg('Вы уверены что хотите закрыть программу?',mtWarning,[mbOk,mbCancel],0)=mrOk then begin
    if m_nNoKey=1 then Begin
    ...
    if blMenuClose=True then Begin
...
begin
  // Конфигуратор
   ...
   //  m_pDB.FixUspdDescEvent(0,3,EVS_OPEN_CONF,SA_USER_PERMIT_PE);
     ...
     pcGEditor.AdvPages[5].TabVisible :=False; // не отображать вкладку тарфиы
     pcGEditor.AdvPages[6].TabVisible :=False; // не отображать вкладку список подключений
...
begin
    ...
    Begin
     ...
     if m_blIsLocal=True  then pDS.m_swData4 := MTR_LOCAL;
     if m_blIsLocal=False then pDS.m_swData4 := MTR_REMOTE;
     if (m_blIsRemCrc=True)or(m_blIsRemEco=True)or(m_blIsRemC12=True) then Begin SendRemCrcStart(pDS);exit; End;
...
begin
    if m_nStateLr=0 then Begin MessageDlg('Функция временно не доступна.Ожидайте.',mtWarning,[mbOk,mbCancel],0); exit; End;
    if MessageDlg('Освободить каналы(порты)?',mtWarning,[mbOk,mbCancel],0)=mrOk then
    Begin
      with lcCEDReleaseChannels do                 // BO 23.11.18
        LogFile.AddEventGroup(GRP,EVNT,IDCurrentUser,now);
     FillChar(pDS,sizeof(pDS),0);
     pDS.m_swData3 := 0;
     if m_blIsLocal=True  then pDS.m_swData4 := MTR_LOCAL;
     if m_blIsLocal=False then pDS.m_swData4 := MTR_REMOTE;
     if (m_blIsRemCrc=True)or(m_blIsRemEco=True)or(m_blIsRemC12=True) then Begin SendRemCrcStop(pDS);exit; End;
...
var
    ...
    sR    : string;
Begin
  if m_nQsFrame <> nil then
    m_nQsFrame.ClearUnloadButton;
     ...
     Begin
      ...
      case PNID of
           ...
           Begin
             ...
             if ((PTSD=SD_STRET) or (PTSD=SD_TPODS)) then Begin
           ...
           //------------------------------------------------------/
             ...
             End;
           //------------------------------------------------------/
             if (PTSD=SD_QGRUP) or (PTSD=SD_QGSRV) then Begin
              vID := FindQsView;
              if vID=-1 then
              Begin
               m_nQsFrame := TTQweryModule.Create(Owner);
               m_nQsFrame.WindowState:= wsMaximized;
               m_nQsFrame.InitFrame(@m_pDS.trTRI,0);

              End else
              if vID<>-1 then
              Begin
               (self.MDIChildren[vID] as TTQweryModule).Caption:= 'Группы опроса';
               (self.MDIChildren[vID] as TTQweryModule).FormActivate(Owner);
               (self.MDIChildren[vID] as TTQweryModule).InitFrame(@m_pDS.trTRI,0);
               self.MDIChildren[vID].Show;
//               self.MDIChildren[vID].WindowState:= wsMaximized;
               (self.MDIChildren[vID] as TTQweryModule).Visible := True;
               (self.MDIChildren[vID] as TTQweryModule).OnTimeElapsed(self);
              End;
             End;

           End;
           PD_VMETR: if PTSD=SD_VMETR then    // когда кликаешь на номере квартиры
           Begin
              //if m_blIsLocal=True then
...
begin
   if TAbout.m_blRG=False then MessageDlg('Разрешено только владельцу!',mtWarning,[mbOk,mbCancel],0) else
   m_nTPLED.OnSaveGrid;    msbStatBar.Panels.Items[0].Progress.Position := 0;
...
begin
    m_nL2Editor.PChild.PPortID := m_nL2Editor.PMasterIndex;
    m_nL2Editor.PChild.OnSetCmdForMeters;
end;
...
begin
   ...
   nSize := trunc(advToolPannel.Width/4)-3;
...
begin
   ...
   //StaticText6.Left:= advPannCurrState.Left+2;
   //StaticText6.Width:= 110;
   //lbCurrUser.Left := StaticText6.Left+StaticText6.Width+1;
   //lbCurrUser.Width:= 110;


   //StaticText13.Left:= advPannCurrState.Left+2;
   //StaticText13.Width:= 110;
   //lbUserInfo.Left := StaticText6.Left+StaticText6.Width+1;
   //lbUserInfo.Width := 110;

   //lbModemInfo.Left := advPannCurrState.Left+2;
   //lbModemInfo.Width := 110;
  // stGprsState.Left := StaticText6.Left+StaticText6.Width+1;
   stGprsState.Width := 110;

   //lbFreeInfo.Left := stGprsState.Left+stGprsState.Width+1;
   //lbFreeInfo.Width := advPannCurrState.Width - 3;
...
procedure TTKnsForm.advButShemClick(Sender: TObject);
begin
    TSchemEditor.ShowModal;
    TShemLoader.GetPopupMenuInfo(advSchemLoader);
    if m_nIsLightControl = 1 then
      AddPopupLightToMenu(advSchemLoader);
end;
...
begin
   ...
   //FillChar(m_nLightInfo,0,sizeof(SL3LIGHTSTRUCT));
   {
   m_nLightInfo.m_nHDOn := 0;
   m_nLightInfo.m_nMDOn := 0;
   m_nLightInfo.m_nHDOff := 0;
   m_nLightInfo.m_nMDOff := 0;

   m_nLightInfo.m_nHR1On := 0;
   m_nLightInfo.m_nMR1On := 0;
   m_nLightInfo.m_nHR1Off := 0;
   m_nLightInfo.m_nMR1Off := 0;

   m_nLightInfo.m_nHR2On := 0;
   m_nLightInfo.m_nMR2On := 0;
   m_nLightInfo.m_nHR2Off := 0;
   m_nLightInfo.m_nMR2Off := 0;

   m_nLightInfo.m_nDAuto := false;
   m_nLightInfo.m_nDChecked := false;
   m_nLightInfo.m_nDCHange := false;
   m_nLightInfo.m_nR1Auto := false;
   m_nLightInfo.m_nR1Checked := false;
...
procedure TTKnsForm.OnSetTyps(Sender: TObject);
begin
     m_nL2Editor.PChild.OnTypeSet;
end;
...
procedure TTKnsForm.cbm_nRegionChange(Sender: TObject);
begin
   AbonAddress.ChangeTRegion;
end;

procedure TTKnsForm.cbm_nDepartChange(Sender: TObject);
begin
   AbonAddress.ChangeTDepartament;
end;

procedure TTKnsForm.cbm_nTownChange(Sender: TObject);
begin
   AbonAddress.ChangeTTown;
end;

procedure TTKnsForm.cbm_nStreetChange(Sender: TObject);
begin
   AbonAddress.ChangeTStreet;
end;

procedure TTKnsForm.BtnRefreshTreeClick(Sender: TObject);
var
    Fl   : TINIFile;
begin
    try
...
procedure TTKnsForm.StaticText6MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TTKnsForm.advOnTestCalendarClick(Sender: TObject);
begin
    //if m_blCL3SznEditor=True then
    if advOnTestCalendar.ImageIndex=21 then
    Begin
     if MessageDlg('Выполнить тестирование календаря с сохранением протокола?',mtWarning,[mbOk,mbCancel],0)=mrOk then
     Begin
      advOnTestCalendar.ImageIndex := 17;
      m_nSZED.OnTestCalendar;
     End;
    End else
    if advOnTestCalendar.ImageIndex=17 then
    Begin
     if MessageDlg('Остановить тестирование календаря?',mtWarning,[mbOk,mbCancel],0)=mrOk then
     Begin
      advOnTestCalendar.ImageIndex := 21;
      m_nSZED.OnTestCalendar;
     End;
    End;
end;
...
procedure TTKnsForm.OnSetCmdForMeter(Sender: TObject);
begin
    m_nL2Editor.PChild.PPortID := m_nL2Editor.PMasterIndex;
    m_nL2Editor.PChild.OnSetCmdForMeter;
end;
{
Procedure TFrmMainMT.btAddXLSUserClick(Sender : TObject);
Var
  nGroupName, sID   : String;
  pUsTbl            : USER;
  entity            : USERGROUP;
  i                 : Integer;
Begin
  Try
    Page := 0;
    If Integer(Excel) = 0 Then
      Excel := CreateOleObject('Excel.Application');
  Except
    MessageDlg('На компьютере отсутствует MS Office Excell или не та версия', mtWarning, [mbOK], 0);
  End;
  odOpenShem.DefaultExt := 'xls';
  odOpenShem.Filter := 'список|*.xls|все файлы|*.*';
  odOpenShem.InitialDir := ExtractFilePath(Application.Exename);
  If odOpenShem.Execute Then Begin
    m_sUserFile := odOpenShem.FileName;
    Excel.Application.EnableEvents := false;
    Excel.WorkBooks.Add(m_sUserFile);
    Excel.ActiveSheet.PageSetup.Orientation := 2;
    connection.DelUserGroups(pUsTbl.USERGROUPID);
    entity.NAMEGROUP := Excel.ActiveSheet.Cells[1, 1].Value;
    ;
    If connection.IsGroupEx(entity.NAMEGROUP) <> -1 Then Begin
      If MessageDlg('Внимание! Группа с названием: ' + entity.NAMEGROUP + ' уже существует. Продолжить?', mtWarning, [mbOK], 0) <> mrOK Then Begin
        exit;
        If Excel.Visible Then Excel.Visible := false;
        Excel.Quit;
        Excel := Unassigned;
      End;
    End;
    connection.AddUserGroups(entity);
    pUsTbl.USERGROUPID := connection.GetLastGroupID;
    i := 5;
    While String(Excel.ActiveSheet.Cells[i, 1].Value) <> '' Do Begin
      pUsTbl.ID := i;
      FillChar(pUsTbl, 0, sizeof(USER));
      If GetUserTable(pUsTbl) = True Then
        connection.AddUsers(pUsTbl);
      i := i + 1;
    End;
    UpdateTreeUser(0);
  End;
  ...
  Excel := Unassigned;
End;
Function TFrmMainMT.GetUserTable(Var pUsTbl : USER) : Boolean;
Var
  sFio              : String;
Begin
  Try
    sFio := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 2].Value) + ' ';
    With pUsTbl Do Begin
      NAME := GetParseWord(2, sFio);
      LASTNAME := GetParseWord(3, sFio);
      MIDDLENAME := GetParseWord(1, sFio);
      DATEBIRTH := m_slYearList.IndexOf(Excel.ActiveSheet.Cells[pUsTbl.ID, 3].Value);
      EDUCATION := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 4].Value);
      m_sSpecDipl := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 5].Value) + String(Excel.ActiveSheet.Cells[pUsTbl.ID, 6].Value);
      m_nRazryad := Integer(Excel.ActiveSheet.Cells[pUsTbl.ID, 7].Value);
      m_nTBGroup := Integer(Excel.ActiveSheet.Cells[pUsTbl.ID, 8].Value);
      WORKPLACE := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 10].Value);
      POST := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 11].Value) + String(Excel.ActiveSheet.Cells[pUsTbl.ID, 12].Value);
      m_nStag := Integer(Excel.ActiveSheet.Cells[pUsTbl.ID, 13].Value);
      m_sHomeAddr := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 14].Value);
      DATEREG := TDateTime(Excel.ActiveSheet.Cells[pUsTbl.ID, 15].Value);
      If DATEREG = 0 Then DATEREG := Now;
      m_sKomNumber := String(Excel.ActiveSheet.Cells[pUsTbl.ID, 16].Value);
      //LGN         := Copy(NAME,0,2)+Copy(LASTNAME,0,2)+Copy(MIDDLENAME,0,2);
      LGN := 'Энерго';
      PWD := Copy(NAME, 0, 1) + Copy(LASTNAME, 0, 1) + Excel.ActiveSheet.Cells[pUsTbl.ID, 3].Value;
      RULE := 3;
    End;
    Result := True;
  Except
...
procedure TTKnsForm.AdvToolBarButton101Click(Sender: TObject);
Var
  nGroupName, sID   : String;
  pTable : SL3GROUPTAG;
  i : Integer;
...
Begin
  ...
  OpenDialog1.Filter := 'список|*.xls|все файлы|*.*';
  OpenDialog1.InitialDir := ExtractFilePath(Application.Exename);
  If OpenDialog1.Execute Then
  Begin
    m_sUserFile := OpenDialog1.FileName;
    Excel.Application.EnableEvents := false;
    Excel.WorkBooks.Add(m_sUserFile);
    Excel.ActiveSheet.PageSetup.Orientation := 2;

    m_pDB.GetAbonMetersTable(4,pTable);
    dsTIME := EncodeDate(2014,2,1);
    for i:=0 to pTable.Item.Count-1 do
    Begin
     if pTable.Item.Items[i].m_swVMID<>377 then
     Begin
      dValue := Double(Excel.ActiveSheet.Cells[5+i,11].Value);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,0,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,1,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,2,QRY_NAK_EN_MONTH_EP,0);
     End;
    End;

    cDateTimeR.DecMonth(dsTIME);
    for i:=0 to pTable.Item.Count-1 do
    Begin
     if pTable.Item.Items[i].m_swVMID<>377 then
     Begin
     dValue := Double(Excel.ActiveSheet.Cells[5+i,11-2].Value);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,0,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,1,QRY_NAK_EN_MONTH_EP,dValue+dEV);
    ...
    Begin
     if pTable.Item.Items[i].m_swVMID<>377 then
     Begin
     dValue := Double(Excel.ActiveSheet.Cells[5+i,11-4].Value);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,0,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,1,QRY_NAK_EN_MONTH_EP,dValue+dEV);
    ...
    Begin
     if pTable.Item.Items[i].m_swVMID<>377 then
     Begin
     dValue := Double(Excel.ActiveSheet.Cells[5+i,11-6].Value);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,0,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,1,QRY_NAK_EN_MONTH_EP,dValue+dEV);
    ...
    Begin
     if pTable.Item.Items[i].m_swVMID<>377 then
     Begin
     dValue := Double(Excel.ActiveSheet.Cells[5+i,11-8].Value);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,0,QRY_NAK_EN_MONTH_EP,dValue+dEV);
      dEV := random(10000)/10000;
      m_pDB.AddToBaseM(dsTIME,pTable.Item.Items[i].m_swVMID,1,QRY_NAK_EN_MONTH_EP,dValue+dEV);
...
procedure TTKnsForm.btTehnoSZClick(Sender: TObject);
begin
  if btTehnoSZ.Down=True then
   m_nSZED.OpenInfo else
  if btTehnoSZ.Down=False then
   m_nSZED.CloseInfo else
end;

procedure TTKnsForm.btTehnoTPClick(Sender: TObject);
begin
  if btTehnoTP.Down=True then
   m_nTPLED.OpenInfo else
  if btTehnoTP.Down=False then
   m_nTPLED.CloseInfo else
end;

procedure TTKnsForm.btTehnoTPRClick(Sender: TObject);
begin
  if btTehnoTPR.Down=True then
   m_nTPLED.PChild.OpenInfo else
  if btTehnoTPR.Down=False then
   m_nTPLED.PChild.CloseInfo else
end;
...
begin
    ...
    Begin
     m_nQsFrame := TTQweryModule.Create(Owner);
     m_nQsFrame.InitFrameADD(@m_pRPDS.trTRI,0);
    End else
    if vID<>-1 then
    Begin
     (self.MDIChildren[vID] as TTQweryModule).Caption:= 'Группы опроса';
     (self.MDIChildren[vID] as TTQweryModule).InitFrameADD(@m_pRPDS.trTRI,0);
     self.MDIChildren[vID].Show;
    End;
end;
...
procedure TTKnsForm.RepErrorRegionDay(index : integer; Name : string);    // ?????? что она делает?
begin
     m_nL3RepFrame := TTRepPower.Create(Owner);
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                    RepErrorRegionDay(infRegion^.m_nRegionID);
                end;
            end;
...
begin
    DecodeTime(Now, _hh, _mm, _ss, _mmmm);
    if (_hh = 11)and (_mm = 15)then      //          jkljkl
    begin
        if (_ss = 1) or (_ss = 2) or (_ss = 3) then
        begin
            m_DB := m_pDB.DynConnect(0); // m_DB - объект подключения к БД
  ...
  {$ENDIF}
{*************************************************************************************************************************
* Процедура редактирования email, на которые будет рассылка отчета о неисправностях
* Form1.ShowModal; - вызывает форму Unit1, в которой можно отправить тестовое письмо
* Перед переходом на эту форму, если не сформированы отчеты за прошлый день, они
* сформируются.
*************************************************************************************************************************}
procedure TTKnsForm.MailTouchClick(Sender: TObject);
var
Year, Month, Day: word;
str : string;
    infRegion  : ^SL3REGION; // инфа о регионах
...
begin
    ...
    tampList := regionList.LockList; // tampList хранит результат запроса id - регионов
    for i:=0 to tampList.Count-1 do
    begin                                                                                        //  'Отчет_об_ошибках_'+ infRegion^.m_nRegNM + '_на_' + str
        infRegion := tampList[i];
        if (infRegion^.m_sbyEnable = 1) and (not FileExists(ExtractFilePath(Application.Exename) + 'ExportData\Отчет_об_ошибках_' + infRegion^.m_nRegNM + '_на_' + str)) then
        begin
            RepErrorRegionDay(infRegion^.m_nRegionID, infRegion^.m_nRegNM);
        end;
    end;
...
procedure TTKnsForm.OPC_BTNClick(Sender: TObject);
begin
    Form3.ShowModal;
end;
...
procedure TTKnsForm.btn1Click(Sender: TObject);
var QQ  : TQueryQuality;
    MDQ : TQQData;  // для месяца
    DDQ : TQQData;  // для дня
    CDQ : TQQData;  // для текущая
    d   : Double;
begin
  QQ := TQueryQuality.Create;
//  QQ.SetQueryQuality(3063);  // получить все 3 записи
//  QQ.SetQueryQualityInNil(3063);  // обнулить запись
  MDQ := QQ.GetQQMonths(3063);  // получить только месяц
  MDQ.Dats := MDQ.Dats + 1000;
  MDQ.Stat := MDQ.Stat + 10;
  MDQ.Perc := MDQ.Perc - 50;
  QQ.SetQQMonths(3063, MDQ);   // сохранить только месяц (остальные значения останутся неизменными)
  DDQ := QQ.GetQQDays(3063);    // получить только день
  DDQ.Dats := DDQ.Dats + 3000;
  DDQ.Stat := DDQ.Stat + 50;
  DDQ.Perc := DDQ.Perc - 1;
  QQ.SetQQDays(3063, DDQ);     // сохранить только день  (остальные значения останутся неизменными)
  CDQ := QQ.GetQQCurrs(3063);  // получить только текущая
  CDQ.Dats := CDQ.Dats + 5000;
  CDQ.Stat := CDQ.Stat + 100;
  CDQ.Perc := CDQ.Perc - 99;
  QQ.SetQQCurrs(3063, CDQ);    // сохранить только текущая  (остальные значения останутся неизменными)
  d:=QQ.GetPercent(4404);
  QQ.Destroy;
end;
...
begin
  ...
  if T =  TimeToStr(ArchBaseCopy.tmm_dtEnterArchTime.Time) then begin
    ArchBaseCopy.Show;
    ArchBaseCopy.miHandExportClick(Sender);
  end;
end;

procedure TTKnsForm.ButtonSendClick(Sender: TObject);
...
begin
  ...
  SQMD.m_snPrmID := 800;
  SQMD.m_snResult := 900;
  s := SenderClass.ToString(SQMD);
  SenderClass.Send(handle, s);
end;
...
procedure TTKnsForm.mnuRasxMonthPeriodClick(Sender: TObject);
begin
  RasxMonthPeriod.ShowModal;
end;

procedure TTKnsForm.mnCorrectClick(Sender: TObject);
begin
//  knslRepCorrect.fRepCorrect1.init;
  knslRepCorrect.ShowModal;
end;

procedure TTKnsForm.FormShow(Sender: TObject);
begin
  //
end;
