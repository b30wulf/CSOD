...
type
    ...
    _last = packed record
      ...
      value   : DWORD;           // Мощность (за 15, 30, 45, 60 минут) кратно 1 Wh
      CRC     : WORD;
    End;


    CE9STKVer16Meter = class(CMeter)
    private
        m_QFNC          : BYTE;
 ...
 *
 ******************************************************************************}
constructor CE9STKVer16Meter.Create;
Begin
 ...
 *
 ******************************************************************************}
destructor CE9STKVer16Meter.Destroy;
Begin
    inherited;
    m_bData.Destroy;
End;

procedure CE9STKVer16Meter.RunMeter;
Begin
    m_nAutoTMR.RunTimer;
end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer16Meter.InitMeter(var pL2:SL2TAG);
  Var
  Year, Month, Day : Word;  
Begin
  IsUpdate := 0;
...
function CE9STKVer16Meter.SelfHandler(var pMsg:CMessage):Boolean;
begin
    Result := False;
 ...
 ******************************************************************************}
function CE9STKVer16Meter.LoHandler(var pMsg:CMessage):Boolean;
begin
  Result := true;
  case pMsg.m_sbyType of
    ...
    PH_DATA_IND:
    Begin
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer16::>Inp DRQ:',@pMsg);

      if not IsValidMessage(pMsg) then
      begin
        TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer16::>Error ControlField:',@pMsg);
        Result := false;
        exit;
      end;
 ...
 ******************************************************************************}
function CE9STKVer16Meter.HiHandler(var pMsg:CMessage):Boolean;
begin
  Result := False;
  ...
  case pMsg.m_sbyType of
    ...
    Begin
      ...
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer16::>Out DRQ:',@pMsg);
 ...
 *      @return Boolean 
 ******************************************************************************}
function  CE9STKVer16Meter.CRC8(var _Buff : array of BYTE; _SetCRC : Boolean) : BYTE;
var
    i, l_Count : integer;
begin
  Result  := 0;
 ...
 *      @return Boolean 
 ******************************************************************************}
function CE9STKVer16Meter.IsValidMessage(var pMsg : CMessage) : Boolean;
var
    l_DataLen : WORD;
begin
    Result := false;
    l_DataLen :=  pMsg.m_sbyInfo[1];
{
    ...
    begin
        TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer16 Ошибка CRC! Выход!');
        exit;
    end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer16Meter.FillMessageHead(var pMsg : CHMessage; length : word);
begin
...
procedure CE9STKVer16Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _Date : TDateTime);
var
 ...
 ******************************************************************************}
procedure CE9STKVer16Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _WriteDate : Boolean);
var
    l_Year, l_Month, l_Day,
    l_Hour, l_Min, l_Second, l_MS : word;
begin
 ...
 *      @return Integer
 ******************************************************************************}
function CE9STKVer16Meter.CMD2ID(_CMDID : BYTE) : BYTE;
Begin
  case _CMDID of
...
procedure CE9STKVer16Meter.HandQryRoutine(var pMsg:CMessage);
Var
    DTS, DTE : TDateTime;
    l_Param        : word;
    szDT         : word;
...
procedure CE9STKVer16Meter.ControlRoutine(var pMsg:CMessage);
Var
    l_Param   : WORD;
    l_StateID : DWORD;
    pDS       : CMessageData;
...
procedure CE9STKVer16Meter.OnFinHandQryRoutine(var pMsg:CMessage);
begin
  ...
  begin
    ...
    //m_IsAuthorized := false;
    OnFinalAction();
    TraceM(2,pMsg.m_swObjID,'(__)CL2MD::>CE9STKVer16 OnFinHandQryRoutine  DRQ:',@pMsg);
    IsUpdate := 0;
  End;
end;

procedure CE9STKVer16Meter.OnEnterAction;
begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer16 OnEnterAction');
...
procedure CE9STKVer16Meter.OnConnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer16 OnConnectComplette');
    m_nModemState := 1;
    if not((m_blIsRemCrc=True)or(m_blIsRemEco=True)) then
        FinalAction;
End;

procedure CE9STKVer16Meter.OnDisconnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer16 OnDisconnectComplette');
    m_nModemState := 0;
End;

procedure CE9STKVer16Meter.OnFinalAction;
begin

    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer16 OnFinalAction');
    FinalAction;
end;

procedure   CE9STKVer16Meter.EncodePacket(var _Buffer : array of BYTE);
var
  l_i : Integer;
begin
...
//
//
function CE9STKVer16Meter.DecompressPacket(data:PByteArray):Boolean;
var
 ...
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ03_OpenSession(var nReq: CQueryPrimitive);
var
  l_FabNum   : DWORD;
  l_Ident    : DWORD;
begin
  ...
  m_nRepTimer.OnTimer(m_nP.m_swRepTime);
  TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer16::>Out DRQ:',@m_nTxMsg);
end;
 ...
 ******************************************************************************)
procedure CE9STKVer16Meter.RES04_OpenSession_OK(var pMsg:CMessage);
begin
  m_IsAuthorized := true;
  ...
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Сессия открыта. Ошибки системы: ' + IntToStr(pMsg.m_sbyInfo[8]) );
  FinalAction();
end;
 ...
 *   @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer16Meter.RES05_OpenSession_Fail(var pMsg:CMessage);
begin
  m_IsAuthorized := false;
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Ошибка: Доступ закрыт!');
  FinalAction();
end;
 ...
 *   @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ05_CloseSession(isTimer: boolean);
begin
  m_QFNC       := $05;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer16Meter.RES05_SessionClosed(var pMsg:CMessage);
begin
  m_IsAuthorized := false;

  ZeroMemory(@m_CODER, 4);
  TraceL (3, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Доступ закрыт!');
  FinalAction();
end;

procedure CE9STKVer16Meter.FillSTKReq(ID, Index: Integer);
begin
   m_QFNC       := $06;
   m_nTxMsg.m_sbyInfo[0] := $7E;
   ...
   FPUT(BOX_L1, @m_nTxMsg);
   m_nRepTimer.OnTimer(m_nP.m_swRepTime);
   TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer16::>Out CMD:',@m_nTxMsg);
end;

procedure CE9STKVer16Meter.CreateSpisSumEnReq(var nReq: CQueryPrimitive);
begin
   {
...
procedure CE9STKVer16Meter.CreateSpisnnNakEnMonReq(var nReq: CQueryPrimitive);
var
    pMsg: CMessage;
begin
    ...
    Begin
     ...
     FillSTKReq(24,nReq.m_swSpecc0-1);
     TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer49::>Чтение нач мес по адресу: ' + IntToStr(nReq.m_swSpecc0));
    End;
end;

procedure CE9STKVer16Meter.CreateSpisnnNakEnDayReq(var nReq: CQueryPrimitive);
begin
   if nReq.m_swSpecc1 = 0 then
   begin
     FinalAction;
...
procedure CE9STKVer16Meter.CreateSumEnergyReq(var nReq: CQueryPrimitive);
begin
   FillSTKReq(12, 0);
end;

procedure CE9STKVer16Meter.CreateSumEnReq(var nReq: CQueryPrimitive);
begin
   m_nTxMsg.m_sbyInfo[0] := $7E;
   m_nTxMsg.m_sbyInfo[1] := $00;
...
procedure CE9STKVer16Meter.ReadMonthEnAns(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   Begin
    ...
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer16::>VMON:'+
...
procedure CE9STKVer16Meter.CreateNakDayEnAns(var nReq: CQueryPrimitive);
var
    pData : CStk43Srez;
...
begin
   ...
   OnFinalAction();
End;
procedure CE9STKVer16Meter.CreateDayEnAns(var nReq: CQueryPrimitive);
var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,nINdex :Integer;
...
begin
   ...
   //if m_bData.FGetSum(dtDate01,pData)=true then
    ...
    OnFinalAction();
End;
procedure CE9STKVer16Meter.Set30Buffer(wShift:Word);
Var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,j,nINdex :Integer;
...
procedure CE9STKVer16Meter.ProcessMonthReq(var nReq: CQueryPrimitive);
...
begin
   ...
   FillSTKReq(24,nReq.m_swSpecc0);
   TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer16::>Чтение месячных архивов по адресу: ' + IntToStr(nReq.m_swSpecc0));
end;



function CE9STKVer16Meter.IsDateTimeEqual(dt1, dt2 : TDateTime): boolean;
...
function CE9STKVer16Meter.GetULongDate(time: Cardinal): TDateTime;
...
function CE9STKVer16Meter.GetUIntDate(time: word): TDateTime;
var yy, mm, dd, hh, nn, ss : word;
begin
   ss := 00;
   nn := (time and $03) * 15;
...
function CE9STKVer16Meter.GetDTFromSM(sm: integer): TDateTime;
...
procedure CE9STKVer16Meter.ProcessSliceReq(var nReq: CQueryPrimitive);
begin
   CreateSliceReq(nReq);
end;
{
...
function  CE9STKVer16Meter.GetNextPointer(nIndex : integer): integer;
Begin
...
procedure CE9STKVer16Meter.CreateSliceReq(var nReq: CQueryPrimitive);
Var
   pData : CStk43Srez;
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ06_GetDataByID(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $06;
  m_QTimestamp := Now();
...
procedure CE9STKVer16Meter.ReadSpisEnerAns(var pMsg: CMessage);  
var
    pVal : _poket;
    i : Integer;
begin
   ...
   Begin
    ...
    //TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer16::>VSpis:'+
...
procedure CE9STKVer16Meter.ReadCurrentParam(var pMsg: CMessage);
var
   pVal : DWORD;
   dbValue : Double;
begin
   ...
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer16::>VCurrPR:'+
...
procedure CE9STKVer16Meter.ReadNakEnMon(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   Begin
    ...
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer16::>VNMON:'+
    ' DT :'+ DateTimeToStr(tempDate)+
    ' Tar:'+ IntToStr(i)+
    ' E+ :'+ FloatTostrF(m_dbNakEn[i+1]-m_dbEnMon[i+1],ffFixed,6,4));
   End;
...
procedure CE9STKVer16Meter.ReadSlicePeriodAns(var pMsg: CMessage);
var perMask : integer;
begin
...
procedure CE9STKVer16Meter.ReadPointSliceAns(var pMsg: CMessage);
...
procedure CE9STKVer16Meter.ReadSliceAns(var pMsg: CMessage);
var
...
begin
   ...
   FinalAction;
   {
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer16::>Srez:'+
...
procedure CE9STKVer16Meter.SendSliceToL3(dValue:Double;wType:Word;dtDate:TDateTime);
begin
...
procedure CE9STKVer16Meter.SendSliceToByIndexL3(dValue:Double;wType:Word;nIndex:Integer;dtDate:TDateTime);
begin
...
procedure CE9STKVer16Meter.SetUnknownSlice(wType:Word);
var tempDate : TDateTime;
begin
...
procedure CE9STKVer16Meter.SetUnknownSliceByIndex(wType:Word;nIndex:Integer);
var tempDate : TDateTime;
begin
   tempDate := GetDTFromSM(nReq.m_swSpecc0);
   FillSaveDataMessage(0, wType, 0, tempDate);
...
procedure CE9STKVer16Meter.ReadSlicesPackAns(var pMsg: CMessage);
begin
...
procedure CE9STKVer16Meter.ReadJrnlAns(var pMsg: CMessage);
var i, uk : integer;
    tDate : TDateTime;
    time  : Cardinal;
begin
...
procedure CE9STKVer16Meter.GetDataAnswer(var pMsg: CMessage);
begin
 ...
 * @param var pMsg : CMessage
 *******************************************************************************)
procedure CE9STKVer16Meter.RES01_GetDataByID_Fail(var pMsg : CMessage);
var
  l_EStr : String;
begin
  ...
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::>' + l_EStr);
  FinalAction();
end;
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ07_GetDateTime(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $07;
  m_QTimestamp := Now();
...
procedure CE9STKVer16Meter.RES07_GetDateTime(var pMsg:CMessage);
var
  nReq : CQueryPrimitive;
...
procedure CE9STKVer16Meter.KorrTime(LastDate : TDateTime);
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer16Meter.RES00_SetDateTime_OK(var pMsg:CMessage);
begin
  if pMsg.m_sbyInfo[2]=0 then
  Begin
   TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Время счетчика установлено: ' + DateTimeToStr(Now()));
...
procedure CE9STKVer16Meter.CreateJrnlEvReq(var nReq: CQueryPrimitive);
begin
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ0F_GetCurrentPower(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0f;
  m_QTimestamp := Now();
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer16Meter.REQ0E_CTRL_SetRelayState(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0E;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer16Meter.RES00_CTRL_SetRelayState_OK(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Операция с контактором выполнена успешно');
  FinalAction();
end;
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer16Meter.RES01_CTRL_SetRelayState_Fail(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer16::> Операция с контактором НЕ ВЫПОЛНЕНА!');
  FinalAction();
end;
 ...
 ******************************************************************************)
procedure CE9STKVer16Meter.ADD_RelayState_CTRLQry(_StateID : WORD);
begin
 ...
 ******************************************************************************}
procedure CE9STKVer16Meter.ADD_Events_GraphQry();
begin
  m_nObserver.ClearGraphQry();
 ...
 * Формирование задания на считывание события
 ******************************************************************************}
procedure CE9STKVer16Meter.ADD_DateTime_Qry();
begin
  m_nObserver.ClearGraphQry();
...
procedure CE9STKVer16Meter.ADD_Energy_Mon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
procedure CE9STKVer16Meter.ADD_Energy_NMon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
 ...
 ******************************************************************************}
procedure CE9STKVer16Meter.AddSresEnergGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i, Srez     : integer;
    h, m, s, ms : word;
    y, d, mn    : word;
...
procedure CE9STKVer16Meter.ADD_Energy_Day_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    //m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
procedure CE9STKVer16Meter.ADD_Energy_NAckDay_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    //m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
function CE9STKVer16Meter.GetYear(month, day: integer): integer;
var yN, mN, dN : word;
    tempDate   : TDateTime;
begin
...
function CE9STKVer16Meter.IsTrueValue(var dbVal:Double):Boolean;
Begin
   Result := True;
