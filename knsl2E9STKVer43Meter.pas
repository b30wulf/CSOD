 ...
 *
 ******************************************************************************}
constructor CE9STKVer43Meter.Create;
Begin
  TraceL(2, m_nP.m_swMID,'(__)CL2MD::> E9STKVer43 Meter Created');
  m_LaEnd := 0;
 ...
 *
 ******************************************************************************}
destructor CE9STKVer43Meter.Destroy;
Begin
    inherited;
    m_bData.Destroy;
End;

procedure CE9STKVer43Meter.RunMeter;
Begin
    m_nAutoTMR.RunTimer;
end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer43Meter.InitMeter(var pL2:SL2TAG);
  Var
  Year, Month, Day : Word;  
Begin
  IsUpdate := 0;
  ...
  {$ENDIF}
  if m_bData=Nil then m_bData := CBox.Create(48*30*sizeof(CStk43Srez));
...
function CE9STKVer43Meter.SelfHandler(var pMsg:CMessage):Boolean;
begin
    Result := False;
 ...
 ******************************************************************************}
function CE9STKVer43Meter.LoHandler(var pMsg:CMessage):Boolean;
begin
  Result := true;
  case pMsg.m_sbyType of
    ...
    PH_DATA_IND:
    Begin
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer43::>Inp DRQ:',@pMsg);

      if not IsValidMessage(pMsg) then
      begin
        TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer43::>Error ControlField:',@pMsg);
        Result := false;
        exit;
      end;
 ...
 ******************************************************************************}
function CE9STKVer43Meter.HiHandler(var pMsg:CMessage):Boolean;
begin
  Result := False;
  ...
  case pMsg.m_sbyType of
    ...
    Begin
      ...
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer43::>Out DRQ:',@pMsg);
 ...
 *      @return Boolean 
 ******************************************************************************}
function  CE9STKVer43Meter.CRC8(var _Buff : array of BYTE; _SetCRC : Boolean) : BYTE;
var
    i, l_Count : integer;
begin
  Result  := 0;
 ...
 *      @return Boolean 
 ******************************************************************************}
function CE9STKVer43Meter.IsValidMessage(var pMsg : CMessage) : Boolean;
var
    l_DataLen : WORD;
begin
    Result := false;
    l_DataLen :=  pMsg.m_sbyInfo[1];
{
    ...
    begin
        TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer43 Ошибка CRC! Выход!');
        exit;
    end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer43Meter.FillMessageHead(var pMsg : CHMessage; length : word);
begin
...
procedure CE9STKVer43Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _Date : TDateTime);
var
 ...
 ******************************************************************************}
procedure CE9STKVer43Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _WriteDate : Boolean);
var
    l_Year, l_Month, l_Day,
    l_Hour, l_Min, l_Second, l_MS : word;
begin
 ...
 *      @return Integer
 ******************************************************************************}
function CE9STKVer43Meter.CMD2ID(_CMDID : BYTE) : BYTE;
Begin
  case _CMDID of
...
procedure CE9STKVer43Meter.HandQryRoutine(var pMsg:CMessage);
Var
    DTS, DTE : TDateTime;
    l_Param        : word;
    szDT         : word;
...
procedure CE9STKVer43Meter.ControlRoutine(var pMsg:CMessage);
Var
    l_Param   : WORD;
    l_StateID : DWORD;
    pDS       : CMessageData;
...
procedure CE9STKVer43Meter.OnFinHandQryRoutine(var pMsg:CMessage);
begin
  ...
  begin
    ...
    //m_IsAuthorized := false;
    OnFinalAction();
    TraceM(2,pMsg.m_swObjID,'(__)CL2MD::>CE9STKVer43 OnFinHandQryRoutine  DRQ:',@pMsg);
    IsUpdate := 0;
  End;
end;

procedure CE9STKVer43Meter.OnEnterAction;
begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer43 OnEnterAction');
...
procedure CE9STKVer43Meter.OnConnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer43 OnConnectComplette');
    m_nModemState := 1;
    if not((m_blIsRemCrc=True)or(m_blIsRemEco=True)) then
        FinalAction;
End;

procedure CE9STKVer43Meter.OnDisconnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer43 OnDisconnectComplette');
    m_nModemState := 0;
End;

procedure CE9STKVer43Meter.OnFinalAction;
begin

    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer43 OnFinalAction');
    FinalAction;
end;

procedure   CE9STKVer43Meter.EncodePacket(var _Buffer : array of BYTE);
var
  l_i : Integer;
begin
...
//
//
function CE9STKVer43Meter.DecompressPacket(data:PByteArray):Boolean;
var
 ...
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ03_OpenSession(var nReq: CQueryPrimitive);
var
  l_FabNum   : DWORD;
  l_Ident    : DWORD;
begin
  ...
  m_nRepTimer.OnTimer(m_nP.m_swRepTime);
  TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer43::>Out DRQ:',@m_nTxMsg);
end;
 ...
 ******************************************************************************)
procedure CE9STKVer43Meter.RES04_OpenSession_OK(var pMsg:CMessage);
begin
  m_IsAuthorized := true;
  ...
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::> Сессия открыта. Ошибки системы: ' + IntToStr(pMsg.m_sbyInfo[8]) );
  FinalAction();
end;
 ...
 *   @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer43Meter.RES05_OpenSession_Fail(var pMsg:CMessage);
begin
  m_IsAuthorized := false;
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::> Ошибка: Доступ закрыт!');
  FinalAction();
end;
 ...
 *   @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ05_CloseSession(isTimer: boolean);
begin
  m_QFNC       := $05;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer43Meter.RES05_SessionClosed(var pMsg:CMessage);
begin
  m_IsAuthorized := false;

  ZeroMemory(@m_CODER, 4);
  TraceL (3, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::> Доступ закрыт!');
  FinalAction();
end;

procedure CE9STKVer43Meter.FillSTKReq(ID, Index: Integer);
begin
   m_QFNC       := $06;
   m_nTxMsg.m_sbyInfo[0] := $7E;
   ...
   FPUT(BOX_L1, @m_nTxMsg);
   m_nRepTimer.OnTimer(m_nP.m_swRepTime);
   TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer43::>Out CMD:',@m_nTxMsg);
end;

procedure CE9STKVer43Meter.CreateSpisSumEnReq(var nReq: CQueryPrimitive);
begin
   {
   ...
   m_nIndex := 0;
   m_ParamID := QRY_ENERGY_SUM_EP;
   FillSTKReq(2, nReq.m_swSpecc1);
end;
procedure CE9STKVer43Meter.CreateSpisnnNakEnMonReq(var nReq: CQueryPrimitive);
var
    pMsg: CMessage;
begin
    ...
    Begin
     ...
     FillSTKReq(17,nReq.m_swSpecc0-1);
     TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer49::>Чтение нач мес по адресу: ' + IntToStr(nReq.m_swSpecc0));
    End;
end;

procedure CE9STKVer43Meter.CreateSpisnnNakEnDayReq(var nReq: CQueryPrimitive);
begin
   if nReq.m_swSpecc1 = 0 then
   begin
     FinalAction;
...
procedure CE9STKVer43Meter.CreateSumEnergyReq(var nReq: CQueryPrimitive);
begin
   FillSTKReq(12, 0);
end;

procedure CE9STKVer43Meter.CreateSumEnReq(var nReq: CQueryPrimitive);
begin
   m_nTxMsg.m_sbyInfo[0] := $7E;
   m_nTxMsg.m_sbyInfo[1] := $00;
...
procedure CE9STKVer43Meter.ReadMonthEnAns(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   Begin
    ...
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer43::>VMON:'+
...
procedure CE9STKVer43Meter.CreateNakDayEnAns(var nReq: CQueryPrimitive);
var
    pData : CStk43Srez;
...
begin
   ...
   OnFinalAction();
End;
procedure CE9STKVer43Meter.CreateDayEnAns(var nReq: CQueryPrimitive);
var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,nINdex :Integer;
...
begin
   ...
   if m_bData.FGetSum(dtDate01,pData)=true then
    ...
    OnFinalAction();
End;
procedure CE9STKVer43Meter.Set30Buffer(wShift:Word);
Var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,j,nINdex :Integer;
...
procedure CE9STKVer43Meter.ProcessMonthReq(var nReq: CQueryPrimitive);
...
begin
   ...
   FillSTKReq(17,nReq.m_swSpecc0);
   TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer43::>Чтение месячных архивов по адресу: ' + IntToStr(nReq.m_swSpecc0));
end;

function CE9STKVer43Meter.IsDateTimeEqual(dt1, dt2 : TDateTime): boolean;
...
function CE9STKVer43Meter.GetULongDate(time: Cardinal): TDateTime;
...
function CE9STKVer43Meter.GetUIntDate(time: word): TDateTime;
var yy, mm, dd, hh, nn, ss : word;
begin
   ss := 00;
   nn := (time and $03) * 15;
...
function CE9STKVer43Meter.GetDTFromSM(sm: integer): TDateTime;
...
procedure CE9STKVer43Meter.ProcessSliceReq(var nReq: CQueryPrimitive);
begin
   CreateSliceReq(nReq);
end;
{
...
function  CE9STKVer43Meter.GetNextPointer(nIndex : integer): integer;
Begin
...
procedure CE9STKVer43Meter.CreateSliceReq(var nReq: CQueryPrimitive);
Var
   pData : CStk43Srez;
...
begin
   ...
   case m_SliceReadSt of
     STSL_STKV116_READ_POINT  : FillSTKReq(8, 0);
     STSL_STKV116_READ_SLICE  : begin
                                 ...
                                 Begin
                                  ...
                                  exit;
                                 End;
                                 FillSTKReq(9, GetNextPointer(nReq.m_swSpecc0));
                                end;
   end;
end;
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ06_GetDataByID(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $06;
  m_QTimestamp := Now();
...
procedure CE9STKVer43Meter.ReadSpisEnerAns(var pMsg: CMessage);  
var
    pVal : _poket;
    i : Integer;
begin
   ...
   Begin
    ...
    //TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer43::>VSpis:'+
...
procedure CE9STKVer43Meter.ReadCurrentParam(var pMsg: CMessage);
var
   pVal : DWORD;
   dbValue : Double;
begin
   ...
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer43::>VCurrPR:'+
...
procedure CE9STKVer43Meter.ReadNakEnMon(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   Begin
    ...
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer43::>VNMON:'+
    ' DT :'+ DateTimeToStr(tempDate)+
    ' Tar:'+ IntToStr(i)+
    ' E+ :'+ FloatTostrF(m_dbNakEn[i+1]-m_dbEnMon[i+1],ffFixed,6,4));
   End;
...
procedure CE9STKVer43Meter.ReadSlicePeriodAns(var pMsg: CMessage);
var perMask : integer;
begin
...
procedure CE9STKVer43Meter.ReadPointSliceAns(var pMsg: CMessage);
...
procedure CE9STKVer43Meter.ReadSliceAns(var pMsg: CMessage);
var
...
begin
   ...
   FinalAction;
   {
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer43::>Srez:'+
...
procedure CE9STKVer43Meter.SendSliceToL3(dValue:Double;wType:Word;dtDate:TDateTime);
begin
...
procedure CE9STKVer43Meter.SendSliceToByIndexL3(dValue:Double;wType:Word;nIndex:Integer;dtDate:TDateTime);
begin
...
procedure CE9STKVer43Meter.SetUnknownSlice(wType:Word);
var tempDate : TDateTime;
begin
...
procedure CE9STKVer43Meter.SetUnknownSliceByIndex(wType:Word;nIndex:Integer);
var tempDate : TDateTime;
begin
   tempDate := GetDTFromSM(nReq.m_swSpecc0);
   FillSaveDataMessage(0, wType, 0, tempDate);
...
procedure CE9STKVer43Meter.ReadSlicesPackAns(var pMsg: CMessage);
begin
...
procedure CE9STKVer43Meter.ReadJrnlAns(var pMsg: CMessage);
var i, uk : integer;
    tDate : TDateTime;
    time  : Cardinal;
begin
...
procedure CE9STKVer43Meter.GetDataAnswer(var pMsg: CMessage);
begin
 ...
 * @param var pMsg : CMessage
 *******************************************************************************)
procedure CE9STKVer43Meter.RES01_GetDataByID_Fail(var pMsg : CMessage);
var
  l_EStr : String;
begin
  ...
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::>' + l_EStr);
  FinalAction();
end;
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ07_GetDateTime(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $07;
  m_QTimestamp := Now();
...
procedure CE9STKVer43Meter.RES07_GetDateTime(var pMsg:CMessage);
var
  nReq : CQueryPrimitive;
...
procedure CE9STKVer43Meter.KorrTime(LastDate : TDateTime);
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer43Meter.RES00_SetDateTime_OK(var pMsg:CMessage);
begin
...
procedure CE9STKVer43Meter.CreateJrnlEvReq(var nReq: CQueryPrimitive);
begin
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ0F_GetCurrentPower(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0f;
  m_QTimestamp := Now();
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer43Meter.REQ0E_CTRL_SetRelayState(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0E;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer43Meter.RES00_CTRL_SetRelayState_OK(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::> Операция с контактором выполнена успешно');
  FinalAction();
end;
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer43Meter.RES01_CTRL_SetRelayState_Fail(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer43::> Операция с контактором НЕ ВЫПОЛНЕНА!');
  FinalAction();
end;
 ...
 ******************************************************************************)
procedure CE9STKVer43Meter.ADD_RelayState_CTRLQry(_StateID : WORD);
begin
 ...
 ******************************************************************************}
procedure CE9STKVer43Meter.ADD_Events_GraphQry();
begin
  m_nObserver.ClearGraphQry();
 ...
 * Формирование задания на считывание события
 ******************************************************************************}
procedure CE9STKVer43Meter.ADD_DateTime_Qry();
begin
  m_nObserver.ClearGraphQry();
...
procedure CE9STKVer43Meter.ADD_Energy_Mon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
procedure CE9STKVer43Meter.ADD_Energy_NMon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
 ...
 ******************************************************************************}
procedure CE9STKVer43Meter.AddSresEnergGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i, Srez     : integer;
    h, m, s, ms : word;
    y, d, mn    : word;
...
procedure CE9STKVer43Meter.ADD_Energy_Day_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
procedure CE9STKVer43Meter.ADD_Energy_NAckDay_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
function CE9STKVer43Meter.GetYear(month, day: integer): integer;
var yN, mN, dN : word;
    tempDate   : TDateTime;
begin
...
function CE9STKVer43Meter.IsTrueValue(var dbVal:Double):Boolean;
Begin
   Result := True;
