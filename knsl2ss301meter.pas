...
type
    ...
    End;
implementation
constructor CSS301Meter.Create;
Begin
End;



procedure CSS301Meter.InitMeter(var pL2:SL2TAG);
Begin
    m_nCounter := 0;
    m_nCounter1:= 0;
    IsUpdate   := false;
...
End;
procedure CSS301Meter.MsgHead(var pMsg:CHMessage; Size:byte);
begin
    pMsg.m_swLen       := Size;
...
procedure CSS301Meter.MsgHeadForEvents(var pMsg:CHMessage; Size:byte);
begin
    pMsg.m_swLen       := Size;
    pMsg.m_swObjID     := m_nP.m_swMID;       //Сетевой адрес счётчика
    ...
    //pMsg.m_sbyTypeIntID:= DEV_COM;       //DEF_COM
    pMsg.m_sbyIntID    := m_nP.m_sbyPortID;
    pMsg.m_sbyServerID := MET_SS301;     //Указать тип счетчика
    pMsg.m_sbyDirID    := m_nP.m_sbyPortID;
end;

procedure CSS301Meter.SetCurrQry;
Begin
    with m_nObserver do
    Begin
     ClearCurrQry;
...
procedure CSS301Meter.AddEnergyDayGraphQry(dt_Date1, dt_Date2 : TDateTime);
...
procedure CSS301Meter.AddEnergyMonthGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
   if m_nOnAutorization = 1 then
...
procedure CSS301Meter.AddSresEnergGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i, Srez     : integer;
    h, m, s, ms : word;
    y, d, mn    : word;
...
procedure CSS301Meter.AddLastSresEnergGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var
    TempDate         : TDateTime;
...
procedure CSS301Meter.AddNakEnDayGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
...
procedure CSS301Meter.AddNakEnMonthGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
   if m_nOnAutorization = 1 then
...
procedure CSS301Meter.AddMaxPower(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i, j        : integer;
begin
   if m_nOnAutorization = 1 then
...
procedure CSS301Meter.AddKorrGraphQry(dt_Date1 : TDateTime; srezN, srezK : byte);
var d, mn, y : word;
    i        : byte;
begin
...
procedure CSS301Meter.AddEventsGraphQry();
var i : integer;
begin
...
procedure CSS301Meter.SetGraphQry;
begin
...
function CSS301Meter.SelfHandler(var pMsg:CMessage):Boolean;
Var
    res : Boolean;
Begin
    res := False;
...
procedure CSS301Meter.Autoriztion;
begin
...
procedure CSS301Meter.KorrTime();
var Year, Month, Day           : word;
    Hour, Min, Sec, mSec       : word;
begin
    //m_nRepTimer.OffTimer;
...
procedure CSS301Meter.WriteDate(var pMsg : CMessage; param : word);
var i, temp          : shortint;
    Year, Month, Day : word;
    TempDate         : TDateTime;
    sm               : shortint;
...
function CSS301Meter.GetDWValue(var pMsg:CMessage;i:integer):DWORD;
begin
   move(pMsg.m_sbyInfo[4 + i*4], Result, sizeof(DWORD));
end;

function CSS301Meter.GetValue(var pMsg:CMessage;i:integer):Single;
Var
...
End;
procedure CSS301Meter.WriteValue(var pMsg:CMessage;i:integer);
Var
    fValue : Double;
begin
...
procedure CSS301Meter.WriteI(var pMsg:CMessage;i:integer);
Var
    fValue : Double;
begin
    fValue := RV(GetValue(pMsg,i));
...
procedure CSS301Meter.WriteU(var pMsg:CMessage;i:integer);
Var
    fValue : Double;
begin
    fValue := RV(GetValue(pMsg,i));
...
procedure CSS301Meter.WriteP(var pMsg:CMessage;i:integer);
Var
    fValue : Double;
begin
    //Operation
...
procedure CSS301Meter.WriteE(var pMsg:CMessage;i:integer);
Var
    Value  : single;
    fValue : double;
    dwTemp : DWORD;
...
function  CSS301Meter.ProcessingData(var pMsg:CMessage; var i:integer) : boolean;
var Year, Month, Day       : word;
...
function CSS301Meter.SendToL3(var pMsg:CMessage) : boolean;
...
function CSS301Meter.LoHandler(var pMsg:CMessage):Boolean;
Var
    res    : Boolean;
    fValue : Single;
Begin
...
procedure CSS301Meter.TestMessage(var pMsg:CMessage);
Var
...
procedure CSS301Meter.GetTimeValue(var nReq: CQueryPrimitive);
var Year, Month, Day       : word;
...
procedure CSS301Meter.HandQryRoutine(var pMsg:CMessage);
Var
...
function CSS301Meter.HiHandler(var pMsg:CMessage):Boolean;
Var
    res          : Boolean;
    //nReq         : CQueryPrimitive;
    tempP        : ShortInt;
Begin
    ...
    case pMsg.m_sbyType of
      ...
      Begin
       ...
       begin
        ...
        if nReq.m_swParamID=QRY_SRES_ENR_EP then MsgHead(m_nTxMsg, 11+4+2+4*2);
        {$ENDIF}
    ...
    Result := res;
End;
procedure CSS301Meter.OnEnterAction;
Begin
    ...
    //FinalAction;
End;
procedure CSS301Meter.OnFinalAction;
Begin
    ...
    FinalAction;
End;
procedure CSS301Meter.OnConnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>SS301 OnConnectComplette');
    m_nModemState := 1;
    FinalAction;
End;
procedure CSS301Meter.OnDisconnectComplette(var pMsg:CMessage);
Begin
    ...
    //FinalAction;
End;
procedure CSS301Meter.OnFinHandQryRoutine(var pMsg:CMessage);
begin
...
function CSS301Meter.GetCommand(byCommand:Byte):Integer;
Var
    res : Integer;
Begin
    case byCommand of
    ...
    Result := res;
End;
function CSS301Meter.CRC(var buf : array of byte; cnt : byte):boolean;
var
    CRChiEl             : byte;
    CRCloEl             : byte;
    i                   : byte;
...
procedure CSS301Meter.RunMeter;
Begin
