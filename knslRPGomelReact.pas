...
type
  TRPGomelReact = class(TForm)
    frGomelReact: TfrReport;
    frDesrGomelReact: TfrDesigner;
...
var
  RPGomelReact: TRPGomelReact;
  IsFirstLoad  : boolean = true;
const
  strEnergy   : array [0..3] of string = ('',
                                          '',
                                          '1.1 Реактивная потребляемая(кВар*ч)',
                                          '1.2 Реактивная отдаваемая(кВар*ч)');
...
procedure TRPGomelReact.PrepareTable;
...
procedure TRPGomelReact.OnFormResize;
Var
    i : Integer;
Begin
    if FsgGrid=Nil then exit;
...
procedure TRPGomelReact.PrintPreview(Date1, Date2 : TDateTime;CountTable:integer;ItemIndMaxCtrl:Integer);
begin
...
function TRPGomelReact.FindTarName(TID : integer; var TarDisc : TM_TARIFFS): string;
var i: integer;
begin
...
function TRPGomelReact.Replace(Str, X, Y: string): string;
var
...
procedure TRPGomelReact.FindValueFormula(tarif:integer;KindEn: byte;mid:string);
var
 stReplace : string;
 i:byte;
begin
...
function TRPGomelReact.GetColorFromTariffs(Srez:byte; var pTable:TM_TARIFFS):Byte;
var SumS                         : word;
...
procedure TRPGomelReact.FillFormula(VMID : integer; Value : Double);
begin
   Fm_nEVL.Variable['v' + IntToStr(VMID) + '_P'] := Value;
end;

procedure TRPGomelReact.FillReportRasx(KEnerg : integer; var Page : TfrPage);
var i, j          : integer;
    Data          : CCDatas;
    PokB, PokE    : array of Double;
    IVB, IVE      : boolean;
begin
   glKindEn := IndEn[2 + KEnerg];
...
procedure TRPGomelReact.FillRasx(var buf : array of double; KindEn: integer; var Page : TfrPage);
var i, j, k, Index   : integer;
    DayCount, res    : integer;
    pTable           : CCDatasEkom;
begin
   glKindEn := IndEn[2 + KindEn];
   Page.ShowBandByName('MasterHeader1');
   ...
   begin
     ...
     PDB.GetEKOM3000GData(DateEnd, DateBeg, StrToInt(FsgGrid.Cells[0, i]),  StrToInt(FsgGrid.Cells[0, i]),
                        QRY_NAK_EN_DAY_RP + KindEn, QRY_NAK_EN_DAY_RP + KindEn, pTable);
...
procedure TRPGomelReact.FillRasxAll(var buf : array of double);
var i, j, k, Index   : integer;
    DayCount, res    : integer;
begin
   for i := 1 to 4 do
...
procedure TRPGomelReact.FindMax;
var i     : integer;
    Index : integer;
begin
   for i := 0 to 4 do
...
procedure TRPGomelReact.FillReportSlice(var Page : TfrPage);
var i, j, k      : integer;
   ...
   begin
     ...
     begin
       if not FDB.GetGraphDatas(DateEnd, DateBeg, StrToInt(FSGGrid.Cells[0, i]),
                                  QRY_SRES_ENR_RP + KindEn, m_pGrData) then
         continue
       else
       begin
...
procedure TRPGomelReact.frGomelActiveManualBuild(Page: TfrPage);
var  i,j,r                         : integer;
...
begin
 ////инициализация формулы
  ...
  { for KindEn := 2 to 3 do
   ...
   FTID_RP   := FDB.LoadTID(QRY_SRES_ENR_RP);
   FTID_RM   := FDB.LoadTID(QRY_SRES_ENR_RM);
   FTID_P    := FDB.LoadTID(QRY_E30MIN_POW_RP);
   PlaneInd := FDB.GetTMTarPeriodsTableGr(GroupID, FTID_RP, m_pTariffs_E[0]);
   FDB.GetTMTarPeriodsTableGr(GroupID, FTID_RM, m_pTariffs_E[1]);
...
procedure TRPGomelReact.frGomelActiveGetValue(const ParName: String;
  var ParValue: Variant);
begin
   ...
   if ParName = 'EnerT1SumP'     then ParValue := DVLS(glEnergSum[0][3]);
   if ParName = 'EnerT2SumP'     then ParValue := DVLS(glEnergSum[0][1]);
   if ParName = 'EnerT3SumP'     then ParValue := DVLS(glEnergSum[0][2]);
