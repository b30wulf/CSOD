...
begin
   ...
   Excel.WorkBooks.Add(ExtractFileDir(application.ExeName)+'\report\ReportKPD.xlt');
...
procedure TReportKPDXL.FormTitle2();
begin
        FindAndReplace('#Ndogovor&',Ndogovor);
        FindAndReplace('#WorksName&',WorksName);
        FindAndReplace('#NameObject&',NameObject);
...
procedure TReportKPDXL.FindValueFormula(tarif:integer;KindEn: byte;mid:string);
var
 stReplace : string;
 i:byte;
begin
...
begin
 glKindEn := IndEn[KindEn];
   ...
   end;{glSum[i] := DVLS(SumEnergy[KindEn][i]);   }
     ShowData2;
