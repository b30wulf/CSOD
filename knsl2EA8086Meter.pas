...
begin
   ...
   ////Проверка на началия в ячейках (расхода а не показаний)/////////
...
begin
   ...
   ///////Конфигурация варианта тарификации///////////
       ...
       begin
        ...
        if (tempBufRead[mass_len-4]=$5A) and (tempBufRead[mass_len-5]=$A5) and (tempBufRead[mass_len-6]=$5A) then
          ...
          begin
           ...
           if (Month_ <> MonthN) and (Year_ <> YearN)then //проверяется месяц и год, когда было последнее программирование
            begin
              State_8086:=2;
              Result:=true;
              test_massiv^ [0] := tempBufRead[Length(tempBufRead) - 4];
...
//        else      { TODO 1 : Проверка на программирования в целом }
//        begin
//          res := false;
//          Result:=res;
//          nres := -3;
...
begin
  ...
  0: if (tempBufRead[12]=0) then
      ...
      begin
       if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'EA8086 ОШИБКА ПАРАМЕТРИЗАЦИИ');
      end;
  1: if (tempBufRead[12]=0) then
      begin
        if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,IntToStr(pTableChannel.m_sMeter[ChannelID-1].M_SWABOID)+' запрограммирован!!!]');
         ...
         if (itemPacket<EA8086_RXPacket.Count-1) then //-1
             ...
             begin
              if (ChannelID>pTableChannel.m_swAmMeter-1) then //-1
                   ...
                   begin
                   if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'EA8086 Channel to kv Packet=['+IntToStr(itemPacket+1)+' запрограммирован!!!]');
                   inc(ChannelID);
                   itemPacket:=0; // инициализируем пакет счетчиков на 0 на следующий канал
  ...
  4:  if (tempBufRead[12]=0) then
      begin
         ...
         mCurrState:=6; // заканчиваем параметризацию по каналам
         Result:=1;
         m_nObserver.AddGraphParam(QRY_LOAD_ALL_PARAMS, 0,0,1,1);
         end
         else
         begin
         mCurrState:=5; // заканчиваем параметризацию параметров
         Result:=1;
         m_nObserver.AddGraphParam(QRY_LOAD_ALL_PARAMS, 0,0,1,1);
         end;
      //end else if(tempBufRead[12]=4) then Result := -1;
      ...
      begin
         if (tarif=0) then
          begin
           ...
           begin
             mCurrState:=4; // заканчиваем параметризацию и обрываем связь 1
             Result:=1;
             m_nObserver.AddGraphParam(QRY_LOAD_ALL_PARAMS, 0,0,1,1);
           end;
      //end else if(tempBufRead[12]=4) then Result := -1;
  ...
  6: if (tempBufRead[12]=0) then
      begin
         mCurrState:=7; // заканчиваем параметризацию и обрываем связь 1
         Result:=1;
      //end else if(tempBufRead[12]=4) then Result := -1;
      end else if(tempBufRead[12] in [1, 2, 3, 4, 5, 6]) then Result := -1;
   ...
   case mCurrState of
     0  :
        begin
             ...
             if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'( '+IntToStr(m_nP.M_SWABOID)+') Кол-во каналов на разъеме'+IntToStr(ChannelID)+' = '+IntToStr(pTableChannel.m_swAmMeter)+' WRITE TIME OK');
        end;
     1  :
        begin
...
begin
  ...
  tempBuf := BufNazvanieRaz('ул. '+PChar(pTable.Items[0].m_sAddress+', дом '+pTable.Items[0].M_SHOUSENUMBER+', корпус '+pTable.Items[0].M_SKORPUSNUMBER+ ', разъём '+IntToStr(nReq.m_swSpecc0)),IntToStr(counKvar));
