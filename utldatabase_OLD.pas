...
uses
    ...
    inifiles,Db,ADODB,knsl5tracer, Dates,utlbox, utlTimeDate,utldynconnect,
    dbtables,knsl3EventBox, Math,IBDataBase,IBQuery;
type
...
Begin
    id := -1;
    if(pTable.m_swStreetID=-1) then strMatch := 'matching (m_swTownID,m_sName)' else
                                    strMatch := 'matching (m_swTownID)';
    with pTable do
    Begin
    strSQL := 'UPDATE OR INSERT INTO SL3STREET'+
              '(m_swTownID,m_sName)'+
              ' VALUES('+
              IntToStr(m_swTownID)+','+
              ''''+m_sName+''''+') '+strMatch;
    End;
    ExecQry(strSQL);
    //strSQL := 'select max(m_swStreetID) as m_swStreetID from SL3STREET';
...
Begin
    ...
    strSQL := 'UPDATE OR INSERT INTO SL3ABON'+
              '('+keyField+'m_nRegionID,M_SWDEPID,M_SWSTREETID,M_SWTOWNID,TPID,m_swPortID,m_sdtRegDate,m_sName,m_sObject,m_sKSP,m_sDogNum,m_sPhone,m_sAddress,m_sEAddress,m_sShemaPath,m_sComment,m_sTelRing,m_sRevPhone,m_sbyVisible,m_sReportVS,m_sLIC,m_strOBJCODE' +
              ',m_sbyEnable,m_sMaxPower,m_sTreeSettings,M_NABONTYPE,M_SMARSHNUMBER,M_SHOUSENUMBER,M_SKORPUSNUMBER,gors,m_sAddrSettings)'+
              ' VALUES('+keyData+
              IntToStr(m_nRegionID)+ ','+
              IntToStr(M_SWDEPID)+ ','+
...
Begin
    ...
    keyField := '';                     
    keyData  := '';
    if(pTable.m_swMID=-1) then Begin strMatch := 'matching (m_swABOID,m_sddFabNum,m_schName)';End else
    begin
     keyField := 'm_swMID,';
...
Begin
    ...
    strSQL := 'INSERT INTO SL3ABON'+
              '(m_swABOID,m_nRegionID,m_swPortID,m_sdtRegDate,m_sName,m_sObject,m_sKSP,m_sDogNum,m_sPhone,m_sAddress,m_sEAddress,m_sShemaPath,m_sComment,m_sTelRing,m_sRevPhone,m_sbyVisible,m_sReportVS,m_sLIC,m_strOBJCODE' +
              ',m_sbyEnable,m_sMaxPower,m_sTreeSettings,M_NABONTYPE,M_SMARSHNUMBER,M_SHOUSENUMBER,M_SKORPUSNUMBER,m_sAddrSettings)'+
              ' VALUES('+
              IntToStr(m_swABOID)+ ','+
              IntToStr(m_nRegionID)+ ','+
              IntToStr(m_swPortID)+ ','+
...
Begin
    res := False;
    if GetVMetersTable(nGID,pTable)=True then
    Begin
...
Begin
    ...
    End;
    ExecQry(strSQL);
    {strSQL := 'select max(ID) as ID from QUERYGROUP';
    if OpenQry(strSQL,nCount)=True then
    Begin
     while not FADOQuery.Eof do Begin
      id := FADOQuery.FieldByName('ID').AsInteger;
...
Begin
    ...
    strSQL := 'UPDATE OR INSERT INTO QGPARAM'+
              ...
              'DAYMASK,MONTHMASK,ENABLE,DEEPFIND,PAUSE,FINDDATA,UNDEEPFIND,UNPATH,UNENABLE,ISRUNSTATUS,RUNSTATUS,ERRORPERCENT)'+
              ' VALUES('+keyData+
              IntToStr(QGID)+ ','+
              IntToStr(PARAM)+ ','+
              ...
              ''''+RUNSTATUS+''''+','+
              FloatToStr(ERRORPERCENT)+') '+strMatch;
    End;
    ExecQry(strSQL);
    Result := 1;
End;
...
Begin
    ...
    {
    if ((nGroup=0)or(nGroup=3))and(nEvents=-1) then
    strSQL := 'SELECT distinct * FROM SL3GROUPTAG as s1,SL3VMETERTAG as s2,SEVENTTTAG as s3'+
              ' WHERE s1.m_sbyGroupID=s2.m_sbyGroupID and s2.m_swVMID=s3.m_swVMID'+
              ' and s1.m_swABOID='+IntToStr(nABOID)+
              //' s3.m_swVMID='+IntToStr(nVMID)+
              ' and s3.m_swGroupID='+IntToStr(nGroup)+
              ' and CAST(s3.m_sdtEventTime AS DATE) BETWEEN '+''''+DateToStr(tm0)+'''' + ' and ' + '''' + DateToStr(tm1) + '''';
    }
    {
    if ((nGroup=0)or(nGroup=3))and(nEvents=-1) then
    strSQL := 'SELECT distinct * SL3ABON as s0,SL3GROUPTAG as s1,SL3VMETERTAG as s2,SEVENTTTAG as s3'+
              ' WHERE s0.m_swABOID=s1.m_swABOID and s1.m_sbyGroupID=s2.m_sbyGroupID and s2.m_swVMID=s3.m_swVMID'+
              ' s0.m_swABOID='+IntToStr(nABOID)+
              ' s3.m_swVMID='+IntToStr(nVMID)+
              ' and s3.m_swGroupID='+IntToStr(nGroup)+
              ' and CAST(s3.m_sdtEventTime AS DATE) BETWEEN '+''''+DateToStr(tm0)+'''' + ' and ' + '''' + DateToStr(tm1) + '''';
    }
...
begin
   Result := false;
   if IsLimitEvent(VMID, CMDID, TID, EvID, Time) then
...
Begin
    ...
    strSQL := 'INSERT INTO SUSERTAG'+
              ...
              ',m_sbyPrmDE,m_sbyPrmPE,m_sbyPrmQE,m_sbyPrmCE,m_sbyPrmGE,m_sbyPrmTE,m_sbyPrmCNE,m_sbyPrmPRE,m_sbyEnable,m_sAllowAbon)'+
               ' VALUES('+
              IntToStr(m_swUID)+ ','+
              IntToStr(m_swSLID)+ ','+
              ''''+DateTimeToStr(m_sdtRegDate)+''''+ ','+
...
Var
    ...
    pMD      : CGMetaData;
Begin
    //SL3PARAMS.m_swVMID=L3CURRENTDATA.m_swVMID
    if nDataType<>-1 then
...
Begin
    ...
    //strSQL := 'DELETE FROM SEVENTTTAG WHERE (CAST(M_SDTEVENTTIME AS DATE) <= ' + '''' + DateToStr(TempDate) + ''''+')';
    //ExecQryD(strSQL);
...
Begin
    ...
    try
     ...
     if FADOQueryDBF.RecordCount>0 then  Begin nCount := FADOQueryDBF.RecordCount; res := True;End;
    except
     res := False;
    end;
    Result := res;
...
Begin
    ...
    //FADOQueryD.Transaction.StartTransaction;
     //FADOQueryD.ExecuteOptions :=[eoAsyncExecute];
     FADOQueryD.Close;
     FADOQueryD.SQL.Clear;
     FADOQueryD.SQL.Add(strSQL);
     FADOQueryD.ExecSQL;
...
Begin
    ...
    //FADOQueryD1.Transaction.StartTransaction;
     ...
     FADOQueryD1.ExecSQL;
     FADOQueryD1.Close;
    //FADOQueryD1.CommitTrans;
    //sCS.Leave;
   except
    //FADOQueryD1.RollbackTrans;
    //sCS.Leave;
    res := False;
   end;
   Result := res;
...
Begin
    ...
    //FADOQueryD2.Transaction.StartTransaction;
     ...
     FADOQueryD2.ExecSQL;
     FADOQueryD2.Close;
    //FADOQueryD2.CommitTrans;
    //sCS.Leave;
   except
    //FADOQueryD2.RollbackTrans;
    //sCS.Leave;
    res := False;
   end;
   Result := res;
...
Begin
    ...
    //FADOQuerySA.Transaction.StartTransaction;
     ...
     FADOQuerySA.ExecSQL;
     FADOQuerySA.Close;
    //FADOQuerySA.CommitTrans;
    //sCS.Leave;
   except
    //FADOQuerySA.RollbackTrans;
    //sCS.Leave;
    res := False;
   end;
   Result := res;
...
Begin
    ...
    //FADOQueryBTI.Transaction.StartTransaction;
     ...
     FADOQueryBTI.Close;
    //FADOQueryBTI.CommitTrans;
    //sCS.Leave;
   except
    //FADOQueryBTI.RollbackTrans;
    //sCS.Leave;
    res := False;
   end;
   Result := res;
...
Begin
    ...
    try
    //sCS.Enter;
    //FADOConnectionDBF.BeginTrans;
     FADOQueryDBF.Close;
     FADOQueryDBF.SQL.Clear;
     FADOQueryDBF.SQL.Add(strSQL);
     FADOQueryDBF.ExecSQL;
     FADOQueryDBF.Close;
    //FADOConnectionDBF.CommitTrans;
    //sCS.Leave;
   except
    //FADOConnectionDBF.RollbackTrans;
    //sCS.Leave;
    res := False;
   end;
   Result := res;
...
begin
    result := true;
    if FADOConnectionMySQL=Nil then
    Begin
     //FADOConnectionMySQL := TIBDataBase.Create(Nil);
     //FADOConnectionMySQL.ConnectionString := m_sMySQLConnStr;
     //FADOConnectionMySQL.LoginPrompt      := False;
...
begin
    ...
    if FADOConnectionMySQL<>Nil then
    Begin
     FADOConnectionMySQL.Destroy;
     FADOConnectionMySQL := Nil;

     if FADOQueryMySQL<>Nil then
     Begin
      FADOQueryMySQL.Destroy;
      FADOQueryMySQL := Nil;
    {
    if FADOConnectionM2F<>Nil then
    Begin
        FADOConnectionM2F.Destroy;
        FADOConnectionM2F := Nil;

        if FADOQueryM2F<>Nil then
        Begin
            FADOQueryM2F.Destroy;
            FADOQueryM2F := Nil;
            result := true;
        End;
        End;
...
Begin
    ...
    try
     ...
     FADOQueryMySQL.SQL.Add(strSQL);
     FADOQueryMySQL.Open;
     if FADOQueryMySQL.RecordCount>0 then  Begin nCount := FADOQueryMySQL.RecordCount; res := True;End;
    except
     res := False;
    end;
    Result := res;
...
Begin
    ...
    try
     ...
     FADOQueryM2F.Open;
     if FADOQueryM2F.RecordCount>0 then  Begin nCount := FADOQueryM2F.RecordCount; res := True;End;
    except
     res := False;
    end;
    Result := res;
