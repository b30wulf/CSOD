...
Begin
    ...
    Begin
     m_blConnectST := True;
     str := 'Удаленный вызов абонента ' + m_sL4ConTag.Items[pMsg.m_swObjID].m_schName+ ' тел. '+m_sL4ConTag.Items[pMsg.m_swObjID].m_schPhone;
     ...
     case pMsg.m_sbyType of
      NL_STARTSRV_REQ   :
       Begin
         ...
         FlbLocal.Caption := 'Идет опрос: '+DateTimeToStr(dtTime);
       End;
      NL_STARTCLEX_REQ:
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbLocal.Caption := 'Расчет успд: '+DateTimeToStr(dtTime);
       End;
      NL_SAVEUPD_REQ:
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbLocal.Caption := 'Данные готовы: '+DateTimeToStr(dtTime);
       End;
     End;
End;
...
//      NL_SETREMML5_REQ  : FcbOutL5.Checked := False;
      ...
      NL_USPDCONN_REQ   : FlbRemote.Caption := 'Устройство функционирует';
      NL_SAVEDBOK_REQ   : FlbRemote.Caption := 'База сжата';
      NL_PACKSTART_REQ  : FlbRemote.Caption := 'Запуск сжатия базы';
      NL_SAVEDB_REQ     : StartPackProc;
      NL_STARTCALC_REQ  : FlbRemote.Caption := 'Расчет';
      NL_WAITPOLL_REQ   : FlbRemote.Caption := 'Ожидание опроса';
      NL_STARTPOLL_REQ  : FlbRemote.Caption := 'Опрос счетчиков';
      NL_UPDGRAPH_REQ   : FlbRemote.Caption := 'Обновление графиков';
      NL_STARTCORR_REQ  : FlbRemote.Caption := 'Запуск коррекции времени';
      NL_FINALCORR_REQ  : FlbRemote.Caption := 'Коррекция времени выполнена';
      NL_STARTRCLC_REQ  : FlbRemote.Caption := 'Запуск перерасчета';
      NL_STOPRCLC_REQ   : FlbRemote.Caption := 'Перерасчет выполнен';
      NL_DATAINFO_REQ   :
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         SetTexSB(3,'rtx:'+IntToStr(pDS.m_swData0)+' rrx:'+IntToStr(pDS.m_swData1));
         FlbRemTime.Caption := DateTimeToStr(dtTime) + ' Разн: ' + FloatToStrF(abs(dtTime-Now)/EncodeTime(0, 0, 0, 1)/1000, ffFixed, 10, 1) + ' сек';
         NotifyConnect;
      ...
      NL_STARTSRV_REQ   :
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbRemote.Caption := 'Идет опрос: '+DateTimeToStr(dtTime);
       End;
      NL_STARTCLEX_REQ:
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbRemote.Caption := 'Расчет успд: '+DateTimeToStr(dtTime);
       End;
      NL_SAVEUPD_REQ:
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbRemote.Caption := 'Данные готовы: '+DateTimeToStr(dtTime);
       End;
      NL_STARTSRVGR_REQ   :
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbRemote.Caption := 'Запуск опроса графиков успд: '+DateTimeToStr(dtTime);
       End;
      NL_STOPSRV_REQ    :
       Begin
         Move(pMsg.m_sbyInfo[0],pDS,sizeof(pDS));
         Move(pDS.m_sbyInfo[0],dtTime,sizeof(dtTime));
         FlbRemote.Caption := 'Останов опроса: '+DateTimeToStr(dtTime);
       End;
      NL_L1INITOK_REQ: FlbRemote.Caption := 'Каналы сознаны...';
      NL_L2INITOK_REQ: FlbRemote.Caption := 'Счетчики сознаны...';
      NL_L3INITOK_REQ: FlbRemote.Caption := 'Точки учета сознаны...';
      NL_L3SDLGO_REQ : FlbRemote.Caption := 'Планировщик запущен...';
      NL_L3SDLST_REQ : FlbRemote.Caption := 'Планировщик остановлен...';
      NL_L3TTMON_REQ : FlbRemote.Caption := 'Запуск перевода времени...';
      NL_L3TTMOF_REQ : FlbRemote.Caption := 'Останов перевода времени...';
      NL_L3CTRON_REQ : FlbRemote.Caption := 'Счетчик подключен...';
      NL_L3CTROF_REQ : FlbRemote.Caption := 'Счетчик отключен...';
      NL_L3RSCOF_REQ : FlbRemote.Caption := 'Статистика отключена...';
      NL_L3RSCON_REQ : FlbRemote.Caption := 'Статистика включена...';
      NL_L3RSSON_REQ : FlbRemote.Caption := 'Статистика очищена...';
      NL_L3RMSOF_REQ : FlbRemote.Caption := 'Удл.Статистика отключена...';
      NL_L3RMSON_REQ : FlbRemote.Caption := 'Удл.Статистика включена...';
      NL_START_TST_REQ : FlbRemote.Caption := 'Диагностика выполненна...';
      NL_STOP_TST_REQ  : FlbRemote.Caption := 'Диагностика остановлена...';
      NL_REM_INIT_REQ  : FlbRemote.Caption := 'Настройки установленны...';
      NL_L3ABOON_REQ : FlbRemote.Caption := 'Абонент подключен...';
      NL_L3ABOOF_REQ : FlbRemote.Caption := 'Абонент отключен...';
      NL_ABOCVRY_REQ : FlbRemote.Caption := 'Опрос абонента...';
      NL_VMTCVRY_REQ : FlbRemote.Caption := 'Опрос счетчика...';
      NL_EXPRTOF_REQ : FlbRemote.Caption := 'Экспорт остановлен...';
      NL_EXPRTON_REQ : FlbRemote.Caption := 'Экспорт запущен...';
      NL_INITVMET_REQ: FlbRemote.Caption := 'Инициализация завершена...';
      NL_QMETPARM_REQ: FlbRemote.Caption := 'Параметр получен...';
      NL_STRTFIND_REQ: FlbRemote.Caption := 'Поиск выполнен...';
      //NL_EXPRTIN_REQ : FlbRemote.Caption := 'Экспорт остановлен...';
...
Begin
     ...
     case pMsg.m_sbyType of
      ...
      QL_DATA_CTRL_REQ: // начало управления  { ukrop ctrl step 2}
      Begin
...
Begin
     case pMsg.m_sbyType of
      ...
      Begin
       ...
       SetTexSB(0,'Опрашивается устройство №'+IntToStr(pMsg.m_swObjID));
       if mL2Module.m_pMeter[pMsg.m_swObjID]<>Nil then
       if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Опрошен учет:'+mL2Module.m_pMeter[pMsg.m_swObjID].m_nP.m_schName);
      ...
      Begin
       ...
       SetTexSB(0,'Информация из канала №'+IntToStr(pMsg.m_swObjID)+' получена');
       if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Все устройства канала опрошены.');
       OnCalculation(pMsg);
      End;
     End;
End;
     ...
     case m_nCF.QueryType of
          ...
          Begin
           ...
           Begin
            ...
            FlbLocal.Caption := 'Расчет';SetTexSB(0,'Вычисление!');SetTexSB(1,'STCALC');
            if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Начало вычислений.');
...
Begin
     case pMsg.m_sbyType of
      ...
      Begin
       ...
       SetTexSB(0,'Графики из устройства №'+IntToStr(pMsg.m_swObjID)+' получены');
       if mL2Module.m_pMeter[pMsg.m_swObjID]<>Nil then
       if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Опрошен учет:'+mL2Module.m_pMeter[pMsg.m_swObjID].m_nP.m_schName);
      End;
      LME_FIN_CHN_POLL_REQ :
...
Begin
     ...
     case pMsg.m_sbyType of
      ...
      Begin
       //TraceL(4,pMsg.m_sbyIntID,'(__)CL3LM::>OnGpllFinChnAction:');
       SetTexSB(0,'Информация из канала №'+IntToStr(pMsg.m_swObjID)+' получена');
       if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Все разрешенные устройства канала опрошены.');
       //SetTexSB(1,'STNULL');
       if ChechState(pMsg)=True then
       Begin
...
Begin
     ...
     SetTexSB(0,'Вычисления завершены');
     SetTexSB(1,'STWAIT');
     if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Вычисления завершены. Ожидание.');
     case pMsg.m_sbyType of
     AL_DATA_FIN_CALC_IND:
      Begin
       FreeL2(pMsg);
       CheckSpaceDB(m_nMaxSpaceDB);  //Не работает тк файл используется
               ...
               case (sQC.m_snPrmID) of
                   0: begin
                       ...
                       if Pos(Str1, Node.Text) > 0 then
                        Node.Text :=Str1+'/ '+ m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Опрос завершен'
                       else
                        Node.Text := m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Опрос завершен';
                       break;
                      end;
                   1: begin
                       Node.ImageIndex := 16;
                       ...
                       if Pos(Str1, Node.Text) > 0 then
                         Node.Text :=Str1+'/ '+ m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Опрос'
                       else
                         Node.Text := m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Опрос';
                       break;                       
                      end;
                   2: begin
                       Node.ImageIndex := 21;
                       ...
                       if Pos(Str1, Node.Text) > 0 then
                         Node.Text := Str1+'/ '+ m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Ожидание опроса(руч.)'
                       else
                         Node.Text := m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Ожидание опроса(руч.)';
                       break;                       
                      end;
                   3: begin
                       Node.ImageIndex   := 21;
                       Node.SelectedIndex:= 21;
                       if Pos(Str1, Node.Text) > 0 then
                        Node.Text := Str1+'/ '+ m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Ожидание опроса(авт.)'
                       else
                        Node.Text := m_pDB.GetGroupQueryName(sQC.m_snSRVID) + ' / ' + DateToStr(date)+' '+TimeToStr(Now)+ ' / Ожидание опроса(авт.)';
                       break;                       
                      end;
               end;
          end;
...
Begin
     ...
     SetTexSB(0,'Дорасчет выполнен');
     SetTexSB(1,'STWAIT');
     if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Дорасчет выполнен. Ожидание.');
     if m_blQrySave=True then
     Begin
      StartPackProc;
      m_blQrySave := False;
...
var
     pDS  : CMessageData;
Begin
     //if m_nCF.IsLocal=True then FlbRemote.Caption := 'Ожидание '+IntToStr(m_nCF.GenPoolPeriod)+' секунд';
     if m_nCF.IsScheduler=False then
     Begin
      FlbLocal.Caption := 'Ожидание '+IntToStr(m_nCF.GenPoolPeriod)+' секунд';
      SetTexSB(0,'Ожидание '+IntToStr(m_nCF.GenPoolPeriod)+' секунд');
     End else
     if m_nCF.IsScheduler=True then
     Begin
      FlbLocal.Caption := 'Ожидание ';
      SetTexSB(0,'Ожидание ');
     End;
     //
...
begin
   ...
   Begin
   {
    ...
    Begin
     ...
     if (res=0) then
     begin
      if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'Разрыв соединения с БД.');
      m_pDB.FullDisconnect;
      m_pDB.Connect;
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Восстановление соединения с БД.')
     end;
    End;
   }
   End;
...
Begin
     ...
     if m_blIsSlave=False then FlbLocal.Caption := 'Поиск данных...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Создание каналов...';
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,NL_L1INITOK_REQ));
End;
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Создание счетчиков...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Создание точек учета...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Перезагрузка УСПД...';
End;
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Запуск планировщика...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Останов планировщика...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Запись установок...';
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,NL_REM_INIT_REQ));
End;
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Запуск перевода времени...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Останов перевода времени...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Установка IP адреса...';
     //if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,NL_L3TTMOF_REQ));
End;
...
Begin
    byCommand:=0;
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if nSET=0 then Begin byCommand:=NL_L3CTROF_REQ;str:='Отключение счетчика...'; End else
     if nSET=1 then Begin byCommand:=NL_L3CTRON_REQ;str:='Подключение счетчика...';End;
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if nSET=0 then Begin byCommand:=NL_L3ABOOF_REQ;str:='Отключение абонента...'; End else
     if nSET=1 then Begin byCommand:=NL_L3ABOON_REQ;str:='Подключение абонента...';End;
     if m_blIsSlave=False then FlbLocal.Caption := str;
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,byCommand));
End;
...
//     if pDS.m_swData4=MTR_LOCAL  then TL2Statistic.OnSetStat(pDS); {TL2Statistic.POnRemStat := pDS.m_swData0};
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if pDS.m_swData0=0 then Begin byCommand:=NL_L3RSCOF_REQ;str:='Отключение статистики...'; End else
     if pDS.m_swData0=1 then Begin byCommand:=NL_L3RSCON_REQ;str:='Подключение статистики...';End;
     if m_blIsSlave=False then FlbLocal.Caption := str;
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,byCommand));
end;
...
//     if pDS.m_swData4=MTR_LOCAL  then TL2Statistic.ResetCounter;
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Обнуление статистики...';
...
//     if pDS.m_swData4=MTR_LOCAL  then TL2Statistic.SetRemState(pDS);
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if pDS.m_swData0=0 then Begin byCommand:=NL_L3RMSOF_REQ;str:='Откл.удаленной статистики...';End else
     if pDS.m_swData0=1 then Begin byCommand:=NL_L3RMSON_REQ;str:='Вкл. удаленной статистики...';End;
     if m_blIsSlave=False then FlbLocal.Caption := str;
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,byCommand));
end;
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Запуск диагностики...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Oстанов диагностики...';
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Останов экспорта...';
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     if m_blIsSlave=False then FlbLocal.Caption := 'Запуск экспорта...';
...
Begin
     ...
     if pDS.m_swData4=MTR_REMOTE then SendRMsg(pMsg);
     byCommand:=NL_QMETPARM_REQ;str:='Запрос параметра...';
     if m_blIsSlave=False then FlbLocal.Caption := str;
     if m_blIsSlave=True  then SendRSMsgM(CreateMSG(BOX_L3,0,DIR_LHTOLMC,byCommand));
end;
...
Begin
     ...
     SetTexSB(0,'Запрашиваются графики из устройства');
     if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Начало опроса графиков.');
     m_nPoolTmr.OffTimer;
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     m_nCalcType:=pDS.m_swData1;
     SetTexSB(0,'Выполняется управление устройстом');
     if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Начало управления устройством.');
     m_nPoolTmr.OffTimer;
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     m_nCalcType:=pDS.m_swData1;
     SetTexSB(0,'Запрашиваются графики из всех устройств');
     if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'Начало опроса графиков.');
     m_nPoolTmr.OffTimer;
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     m_nCalcType:=pDS.m_swData1;
     SetTexSB(0,'Запрашиваются события из всех устройств');
     if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Начало опроса событий.');
     m_nPoolTmr.OffTimer;
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
...
Begin
     ...
     End;
     FlbLocal.Caption := 'Идет опрос: '+DateTimeToStr(Now);
     m_nCF.SchedPause;
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     SetTexSB(0,'Запрашиваются графики из устройства '+IntToStr(pDS.m_swData2));
     SetTexSB(1,'STPLLG');
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
     ...
     End;
     FlbLocal.Caption := 'Запуск опроса графиков: '+DateTimeToStr(m_sALD.m_sdtArchTmpTime);
...
Begin
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     SetTexSB(0,'Выполняются управление устройством '+IntToStr(pDS.m_swData2));
     SetTexSB(1,'STPLLG');
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
      if m_nQrySender[pDS.m_swData3]<>Nil then
     ...
     FlbLocal.Caption := 'Управление устройством ';
     if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,FlbLocal.Caption);
     m_nCF.SchedPause;
     m_nWPoolTmr.OnTimer(60*m_nWPollTime);
End;
...
Begin
     SetTexSB(0,'Запрашиваются графики из всех устройств');
     SetTexSB(1,'STPLAG');
     //if EventBox<>Nil then EventBox.FixEvents(ET_RELEASE,'Начало опроса графиков.');
     Move(pMsg.m_sbyInfo[0],pDS,sizeof(CMessageData));
     if pDS.m_swData4=MTR_LOCAL  then
     Begin
      for i:=0 to MAX_PORT do
...
Begin
     ...
     End;
     FlbLocal.Caption := 'Останов опроса: '+DateTimeToStr(Now);
...
//   (Now - m_nLastCorrTime) / EncodeTime(1, 0, 0, 0) - Время в часах
//   m_nAutoKorrDay / 24  (Время в переводе на с за час)
...
Begin
   {   if (m_nLmeCounter mod m_nUpdateTime)=0 then
      Begin
       ...
       //SetTexSB(7,'Запросов:'+IntToStr(FCHECK(BOX_LOAD)));
     End;
    ...
    //  Begin
     ...
     //  SetTexSB(6,'Запросов:'+IntToStr(szQry)+':'+IntToStr(szBuf));
    //   if m_nCF.QueryType=QWR_QWERY_SRV then
