...
procedure TrpKPDReport.PrepareTable;
...
procedure TrpKPDReport.SetHigthGrid(var sgGrid:TAdvStringGrid;nHigth:Integer);
Var
    i : Integer;
Begin
        for i:=0 to sgGrid.RowCount-1 do Begin sgGrid.Cells[0,i+1] := IntToStr(i+1); sgGrid.RowHeights[i]:= nHigth;End;
End;
procedure TrpKPDReport.OnFormResize;
Var
    i : Integer;
Begin
    if FsgGrid=Nil then exit;
...
procedure TrpKPDReport.PrintPreview(Date : TDateTime;CountTable:integer);
begin
   DateReport := Date;
...
function TrpKPDReport.Replace(Str, X, Y: string): string;
var
...
procedure TrpKPDReport.FindValueFormula(tarif:integer;KindEn: byte;mid:string);
var
 stReplace : string;
 i:byte;
begin
...
procedure TrpKPDReport.FillReport(MID : integer; var Page: TfrPage);
var Data                : CCDatas;
    KindEn              : byte;
...
procedure TrpKPDReport.FillReportTtl(KindEN : byte; var Page : TfrPage);
var i : byte;
begin
...
procedure TrpKPDReport.frReport1ManualBuild(Page: TfrPage);
var i,j                : integer;
...
procedure TrpKPDReport.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TrpKPDReport.frReport1EnterRect(Memo: TStringList;
  View: TfrView);
begin
   if IsLastPage then
