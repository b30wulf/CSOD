...
const
  c_EnergyNames : array[0..3] of string = ('принимаемой активной',
                                           'отдаваемой активной',
                                           'принимаемой реактивной',
                                           'отдаваемой реактивной');

  c_EnergyUnits : array[0..3] of string = ('кВт·ч',
                                           'кВт·ч',
                                           'кВар·ч',
                                           'кВар·ч');
...
begin
  ...
  NDogovor := trim(NDogovor);
   globalTtlMainName := 'Количество ' + c_EnergyNames[KindEnergy] + ' энергии в ' + cDateTimeR.GetNameMonth(Month)
                         + ' ' + IntToStr(Year) + ' года';
...
begin
  ...
  else if ParName = 'TblT0Name'   then ParValue := 'Общее количество '+c_EnergyNames[KindEnergy]+' энергии, ' + c_EnergyUnits[KindEnergy]//+#13#10;
...
begin
  if (_TarName = 'Полупиковая зона') then
    Result := 'Количество '+c_EnergyNames[KindEnergy]+' энергии в часы полупиковой зоны, ' + c_EnergyUnits[KindEnergy]
  else if (_TarName = 'Пиковая зона') then
    Result := 'Количество '+c_EnergyNames[KindEnergy]+' энергии в часы пиковой зоны, ' + c_EnergyUnits[KindEnergy]
  else if (_TarName = 'Ночная зона') then
    Result := 'Количество '+c_EnergyNames[KindEnergy]+' энергии в часы ночной зоны, ' + c_EnergyUnits[KindEnergy]
  else
