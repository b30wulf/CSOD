...
Begin
    // FTreeModuleData.LoadGraph(m_nRCT.m_swVMID);
     ...
     m_pData.m_CRC    := ReturnCRC(m_pData.m_sfValue);   //формирование crc
...
Begin
     ...
     Begin
      ...
      if pGI.m_crc=0 then pTN.m_crc := $40;//Валидность параметра в настройках
      if (pGI.m_sMaskReRead and (nM shl pTN.nSID))=0 then pTN.m_sbyPrecision := pTN.m_sbyPrecision or $40; //Валидность данных
