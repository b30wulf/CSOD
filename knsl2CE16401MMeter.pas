...
type
    ...
    private
        ...
        m_nState      : Byte;
        StateOn       : Boolean;
        vTar          : Boolean;//для сохранения тарифа с нормальным статусом
    public
        // base
        constructor Create;
    ...
    private
        ...
        function    RES0120_ReadDateTime(var pMsg : CMessage):Boolean;  // Оnвет
        ...
        procedure   REQ0120_ReadDateTime(var nReq: CQueryPrimitive);    // Запрос чтение даты/времени
        procedure   REQ0120_CorrTimeReq(var nReq: CQueryPrimitive);     // Запрос корректировка даты/времени
...
begin
  ...
  STIME         := 20;
  byMaxTar      := 5;
  m_nT.B5       := false;//для проверки какая команда выполняется
  m_IsAuthorized := false;
...
begin
  ...
  //      end;
        ...
        case m_QFNC of // FUNCTION
          ...
          $0120 : Result:=ReadDateTimeAns(pMsg);       //Протестировано-> Работает
          $0121 : RES0121_WriteDateTime(pMsg);
          $0130 : RES0130_ReadTariffValue(pMsg);
          $0132 : RES0132_GetCurrentPower(pMsg);
          $0135 : Result:=RES0135_GetEnergyMonth(pMsg);//Протестировано-> Работает
          $0133 : Result:=RES0133_GetEnergyDay(pMsg);  //Протестировано-> Работает
    ...
    case pMsg.m_sbyType of
      ...
      Begin
        ...
        case nReq.m_swParamID of
          ...
          QRY_DATA_TIME      : CreateDateTimeReq(nReq);//REQ0120_ReadDateTime(nReq);   //определения для чтения  времени или корректировки
...
(*******************************************************************************
 * Запрос на чтение времени и даты
 ******************************************************************************)
...
(*******************************************************************************
 * Запрос "Корректировки времени и даты"
 *******************************************************************************)
procedure CCE16401MMeter.REQ0120_CorrTimeReq(var nReq: CQueryPrimitive);
var Year, Month, Day,
    Hour, Min, Sec, Msec : word;
    mCmd : CCommand;
    LenPac:byte; //Длина пакета
begin
    //m_nTxMsg.m_sbyInfo[0]  := StrToInt(m_nP.m_sddPHAddres);
    DecodeDate(Now, Year, Month, Day);
    DecodeTime(Now, Hour, Min, Sec, Msec);   
    Hour:=Hour-3; //-3 Для перевода в GMT У нас +3, делаю -3

    m_QTimestamp := Now();
    m_QFNC       := $0120;
    m_Req        := nReq;
    mCmd.byLen   := 6;   //длина пакета только для времени
    mCmd.byCmd   := $05;  //Команда установки времени
...
begin
    ...
    ReadDate:=EncodeDate(_yy,_mn,_dd) + EncodeTime(_hh,_mm,_ss,0);
//    if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'Разница по времени после корректировки состовляет -> '+DateTimeToStr(ReadDate - Now));
    Result:=True;
end;
    ...
    if res then begin
      ...
      if m_nT.B2 then
        for i := 0 to nCount - 1 do Begin
          ...
          begin
          if vTar=True then
               m_nT.B2:=True;//для всего канала состояния опроса, иначе будет false если все кроме первого не с верным статусом
          Break;
          end;
