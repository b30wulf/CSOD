...
procedure TRPVedomPokaz.SetHigthGrid(var sgGrid:TAdvStringGrid;nHigth:Integer);
Var
    i : Integer;
Begin
...
procedure TRPVedomPokaz.OnFormResize;
Var
    i : Integer;
Begin
...
procedure TRPVedomPokaz.PrepareTable;
...
procedure TRPVedomPokaz.PrintPreview(dt_beg, dt_end : TDateTime);
begin
...
function  TRPVedomPokaz.FindColSlicesFromMask(Mask : int64) : integer;
var i : integer;
begin
   Result := 0;
   for i := 0 to 47 do
...
procedure TRPVedomPokaz.GetMeterInfo(VMID: integer);
...
procedure TRPVedomPokaz.GetPokMetersInfo(GridNote, KindEn: integer);
...
begin
   ...
   if (pTable.Count > 0) and (trunc(pTable.Items[0].m_sTime) = trunc(dt1)) then
     glIzmBeg := FloatToStr(RVLPr(pTable.Items[0].m_sfValue/KT, 5))
//     glIzmBeg := FloatToStr(RVLPr(pTable.Items[0].m_sfValue/KT, MeterPrecision[StrToInt(FsgGrid.Cells[0, GridNote])]{4}))
   else
     glIzmBeg := '-';
   if (pTable.Count > 0) and (trunc(pTable.Items[pTable.Count - 1].m_sTime) = trunc(dt2)) then
     glIzmEnd := FloatToStr(RVLPr(pTable.Items[pTable.Count - 1].m_sfValue/KT, 5))
//     glIzmEnd := FloatToStr(RVLPr(pTable.Items[pTable.Count - 1].m_sfValue/KT, MeterPrecision[StrToInt(FsgGrid.Cells[0, GridNote])]{4}))
   else
     glIzmEnd := '-';
   if (glIzmBeg <> '-') and (glIzmEnd <> '-') then
     glRasxPok := FloatToStr(RVLPr((StrToFloat(glIzmEnd) - StrToFloat(glIzmBeg))*KT, 5))
//     glRasxPok := FloatToStr(RVLPr((StrToFloat(glIzmEnd) - StrToFloat(glIzmBeg))*KT, MeterPrecision[StrToInt(FsgGrid.Cells[0, GridNote])]{4}))
   else
     glRasxPok := '-';
end;

procedure TRPVedomPokaz.GetSlicesInfo(GridNote, KindEn: integer);
...
begin
   ...
   glRasxSl   := FloatToStr(RVLPr(SumEn, {MeterPrecision[StrToInt(FsgGrid.Cells[0, GridNote])]}5));
end;
procedure TRPVedomPokaz.OnClickVedom(ARow, ACol: Integer);
Begin
    m_nCol := -1;
    m_nRow := -1;
    ...
    End;
End;
procedure TRPVedomPokaz.BuildReport(var Page: TfrPage);
...
procedure TRPVedomPokaz.frReport1ManualBuild(Page: TfrPage);
...
begin
   ...
   glDateReport := DateToStr(dt1) + '   ' + DateToStr(dt2);
...
procedure TRPVedomPokaz.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TRPVedomPokaz.frReport1EnterRect(Memo: TStringList;
  View: TfrView);
begin
