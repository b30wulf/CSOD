...
begin
  ...
  // 8.5.2002, Kf, Error corrected. The Devmode structure has
//different size for different
  // printers. So it has to be reallocated when the printer changes.
//But only then, because
  // otherwise settings like papersize get lost.
  if Value = FPrinterIndex then Exit;
  with TPrinterDevice(FPrinters.Objects[Value]) do
