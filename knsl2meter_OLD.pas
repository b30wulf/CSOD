...
Windows, Classes, SysUtils,SyncObjs,stdctrls,comctrls,utltypes,utlbox,utlconst,knsl5config,utlmtimer,
 knsl3observemodule,utldynconnect,knsl3querysender,knsl5tracer,utldatabase,knsl2statistic,knsl3EventBox,utlSpeedTimer,utlTimeDate;
type
    CMeter = class
    //protected
    // m_nP  : SL2TAG;
...
Begin
    inherited;
    if m_nReStart<>Nil  then m_nReStart.Destroy;
    if m_nAutoTMR<>Nil  then m_nAutoTMR.Destroy;
    if m_nRepTimer<>Nil then m_nRepTimer.Destroy;
    if m_nObserver<>Nil then m_nObserver.Destroy;
End;
...
Begin
     ...
     case getTypeParam(data.m_swCMDID) of
          ...
          Begin
           //if isTrueValue(data) then
           dynConnect.AddArchDataNoBlock_8086(fub_num, id_object, data);
          End;
      ...
      Begin
        ...
        Begin
         ...
         Begin
          ...
          Dec(m_byRep);
         End else TraceL(3,m_nP.m_swMID,'(__)CL2MD::>REJECT Not Enable! Active Tranzaction!');
