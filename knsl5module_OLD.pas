...
//    procedure WMNormalClose(var Msg:TMessage);message WM_SENDTOKNSTOP;
  ...
  public
    ...
    m_nQsFrame         : TTQweryModule;    
    { Public declarations }
    procedure RepErrorRegionDay(index : integer);
...
uses knsl4ConfMeterModule, Unit1, knsl3DepartamentIns,knsl5ArchBaseCopy,
     fLogView, Unit3, Knsl5Meter_Replace,knsl3housegen;
...
procedure TTKnsForm.RepErrorRegionDay(index : integer);
begin
     m_nL3RepFrame := TTRepPower.Create(Owner);
     m_nL3RepFrame.CreateRPError(index);//1 здесь надо указать номер региона, который хочешь выгружать либо их в цикле создавать несколько 8, 9, 10
...
begin
  try
   ...
   N99.Enabled:=false;
   //TAbonManager.importHouseTaskManager;
     ...
     begin
      ...
      //     m_nHouseGen.InitTreeRef; //Автообновление дерева
       FreeAndNil(m_nHouseGen); // m_nHouseGen.Destroy;//протестировать
     end;
  finally
     MessageDlg('Абонент загружен! ',mtWarning,[mbOk],0);
