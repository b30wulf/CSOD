...
//     ResDoz:=2;
  ...
  addrBegin := -1;
   if (cmd=QRY_DATA_TIME) then
      objs[i].res := objs[i].runnble.run(gid,kol_Y,kol_N,CPORT,dateBegin)// Запуск только первого запроса в очереди
   else
   objs[i].res := objs[i].runnble.runAsync(gid,kol_Y,kol_N,CPORT,dateBegin); // Запуск только первого запроса в очереди

   for j := i - addrOffset to Pred(i) do
    begin
     objs[j].meter.m_nP.m_swAddrOffset := j - i + addrOffset;
     objs[j].res := objs[j].runnble.findAsync(gid,kol_Y,kol_N,CPORT,dateBegin); // Обработка остальных ответов
    end;
