...
type
    // Количество всего списанных киловат по тарифам
    // CVToken = packed record
    // Таблица указателей на номера
    // Количество списанных ватт по тарифам
    _cntE = packed record
      val : array[0..3] of Double;
				// 1 За месяц текущий
				// 2 За месяц прошедший
				// 3 За день текущий
				// 4 За день прошедший
    End;
    _poket = packed record
      DATETIME : Cardinal;	// Дата и время
      CNT : array[0..5] of _cntE;
      CRC : Word;
    End;
    _pointers = packed record
      UKTABONOFF  : Byte;		// Указатель на ячейку в TABONOFF
      CNTTABONOFF : Byte;		// Число записей в TABONOFF
      UKLAST      : Word;            	// Указатель на ячейку LAST
      CNTLAST     : Word;		// Число записей в LAST
      TARIFNUM    : Byte;		// Номер текущего тарифа
      UKKTRTAB    : Byte;		// Указатель на ячейку в KTRTAB
      CNTKTRTAB   : Byte;		// Число записей в KTRTAB
      UKOPTOTAB   : Byte;		// Указатель на ячейку в OPTOTAB
      CNTOPTOTAB  : Byte;		// Число записей в OPTOTAB
      CRC : Word;
    End;
...
// PHASE[3] - общий по трем фазам
    ...
    _srez = packed record
      DATETIME : Cardinal;
      Energy : array[0..5] of Double;   // энергия по 6-ти квадрантам
      Power : array[0..5] of Double;    // маскимальная средняя мощность за время интеграции
      CRC : Word;
    End;
    ...
    l_DataLen :=  pMsg.m_sbyInfo[1];
{
    ...
    begin
        TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer49 Ошибка CRC! Выход!');
        exit;
    end;
...
begin
  ...
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Сессия открыта. Ошибки системы: ' + IntToStr(pMsg.m_sbyInfo[8]) );
  FinalAction();
end;
...
begin
  m_IsAuthorized := false;
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Ошибка: Доступ закрыт!');
  FinalAction();
end;
...
begin
  ...
  ZeroMemory(@m_CODER, 4);
  TraceL (3, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Доступ закрыт!');
  FinalAction();
end;
...
begin
   ...
   FillSTKReq(4,0);
   TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer49::>Чтение месячных архивов по адресу: ' + IntToStr(nReq.m_swSpecc0));
end;
...
begin
   ...
   FillSTKReq(4,0);
   TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer49::>Чтение дневных архивов по адресу: ' + IntToStr(nReq.m_swSpecc0));
end;
...
begin
  ...
  Begin
   TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Время счетчика установлено: ' + DateTimeToStr(Now()));
...
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Операция с контактором выполнена успешно');
  FinalAction();
end;
...
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer49::> Операция с контактором НЕ ВЫПОЛНЕНА!');
  FinalAction();
end;
