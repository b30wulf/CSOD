...
type
    ...
    _pointers = packed record
      ...
      DEFLDAnum  : Word;            	// Указатель на ячейку LAST
      DEFLDAuk   : Word;		// Число записей в LAST
      CRC        : Word;
    End;
 ...
 *
 ******************************************************************************}
constructor CE9STKVer18Meter.Create;
Begin
  TraceL(2, m_nP.m_swMID,'(__)CL2MD::> E9STKVer18 Meter Created');
  m_LaEnd := 0;
 ...
 *
 ******************************************************************************}
destructor CE9STKVer18Meter.Destroy;
Begin
    inherited;
    m_bData.Destroy;
End;

procedure CE9STKVer18Meter.RunMeter;
Begin
    m_nAutoTMR.RunTimer;
end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer18Meter.InitMeter(var pL2:SL2TAG);
  Var
  Year, Month, Day : Word;  
Begin
  IsUpdate := 0;
  ...
  TraceL(2,m_nP.m_swMID,'(__)CL2MD::>E9CTK Ver18   Meter Created:'+
...
function CE9STKVer18Meter.SelfHandler(var pMsg:CMessage):Boolean;
begin
    Result := False;
 ...
 ******************************************************************************}
function CE9STKVer18Meter.LoHandler(var pMsg:CMessage):Boolean;
begin
  Result := true;
  case pMsg.m_sbyType of
    ...
    PH_DATA_IND:
    Begin
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer18::>Inp DRQ:',@pMsg);

      if not IsValidMessage(pMsg) then
      begin
        TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer18::>Error ControlField:',@pMsg);
        Result := false;
        exit;
      end;
      EncodePacket(pMsg.m_sbyInfo);
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer18::>Inp DRQ_1:',@pMsg);
{
 ...
 ******************************************************************************}
function CE9STKVer18Meter.HiHandler(var pMsg:CMessage):Boolean;
begin
  Result := False;
  ...
  case pMsg.m_sbyType of
    ...
    Begin
      ...
      TraceM(2,pMsg.m_swObjID,'(__)CE9STKVer18::>Out DRQ:',@pMsg);
 ...
 *      @return Boolean 
 ******************************************************************************}
function  CE9STKVer18Meter.CRC8(var _Buff : array of BYTE; _SetCRC : Boolean) : BYTE;
var
    i, l_Count : integer;
begin
  Result  := 0;
 ...
 *      @return Boolean 
 ******************************************************************************}
function CE9STKVer18Meter.IsValidMessage(var pMsg : CMessage) : Boolean;
var
    l_DataLen : WORD;
begin
    Result := false;
    l_DataLen :=  pMsg.m_sbyInfo[1];
{
    ...
    begin
        TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer18 Ошибка CRC! Выход!');
        exit;
    end;
 ...
 *
 ******************************************************************************}
procedure CE9STKVer18Meter.FillMessageHead(var pMsg : CHMessage; length : word);
begin
   ...
   pMsg.m_sbyServerID   := MET_STKVer18;
   pMsg.m_sbyDirID      := IsUpdate;
   SendOutStat(pMsg.m_swLen);
end;

procedure CE9STKVer18Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _Date : TDateTime);
var
 ...
 ******************************************************************************}
procedure CE9STKVer18Meter.FillSaveDataMessage(_Value : double; _ParamID : byte; _Tariff : byte; _WriteDate : Boolean);
var
    l_Year, l_Month, l_Day,
    l_Hour, l_Min, l_Second, l_MS : word;
begin
 ...
 *      @return Integer
 ******************************************************************************}
function CE9STKVer18Meter.CMD2ID(_CMDID : BYTE) : BYTE;
Begin
  case _CMDID of
...
procedure CE9STKVer18Meter.HandQryRoutine(var pMsg:CMessage);
Var
    DTS, DTE : TDateTime;
    l_Param        : word;
    szDT         : word;
...
procedure CE9STKVer18Meter.ControlRoutine(var pMsg:CMessage);
Var
    l_Param   : WORD;
    l_StateID : DWORD;
    pDS       : CMessageData;
...
procedure CE9STKVer18Meter.OnFinHandQryRoutine(var pMsg:CMessage);
begin
  ...
  begin
    ...
    //m_IsAuthorized := false;
    OnFinalAction();
    TraceM(2,pMsg.m_swObjID,'(__)CL2MD::>CE9STKVer18 OnFinHandQryRoutine  DRQ:',@pMsg);
    IsUpdate := 0;
  End;
end;

procedure CE9STKVer18Meter.OnEnterAction;
begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer18 OnEnterAction');
...
procedure CE9STKVer18Meter.OnConnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer18 OnConnectComplette');
    m_nModemState := 1;
    if not((m_blIsRemCrc=True)or(m_blIsRemEco=True)) then
        FinalAction;
End;

procedure CE9STKVer18Meter.OnDisconnectComplette(var pMsg:CMessage);
Begin
    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer18 OnDisconnectComplette');
    m_nModemState := 0;
End;

procedure CE9STKVer18Meter.OnFinalAction;
begin

    TraceL(2,m_nP.m_swMID,'(__)CL2MD::>CE9STKVer18 OnFinalAction');
    FinalAction;
end;

procedure   CE9STKVer18Meter.EncodePacket(var _Buffer : array of BYTE);
var
  l_i : Integer;
begin
...
//
//
function CE9STKVer18Meter.DecompressPacket(data:PByteArray):Boolean;
var
 ...
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ03_OpenSession(var nReq: CQueryPrimitive);
var
  l_FabNum   : DWORD;
  l_Ident    : DWORD;
begin
  ...
  m_nRepTimer.OnTimer(m_nP.m_swRepTime);
  TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer18::>Out DRQ:',@m_nTxMsg);
end;
 ...
 ******************************************************************************)
procedure CE9STKVer18Meter.RES04_OpenSession_OK(var pMsg:CMessage);
begin
  m_IsAuthorized := true;
  ...
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Сессия открыта. Ошибки системы: ' + IntToStr(pMsg.m_sbyInfo[8]) );
  FinalAction();
end;
 ...
 *   @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer18Meter.RES05_OpenSession_Fail(var pMsg:CMessage);
begin
  m_IsAuthorized := false;
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Ошибка: Доступ закрыт!');
  FinalAction();
end;
 ...
 *   @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ05_CloseSession(isTimer: boolean);
begin
  m_QFNC       := $05;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 ******************************************************************************)
procedure CE9STKVer18Meter.RES05_SessionClosed(var pMsg:CMessage);
begin
  m_IsAuthorized := false;

  ZeroMemory(@m_CODER, 4);
  TraceL (3, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Доступ закрыт!');
  FinalAction();
end;

procedure CE9STKVer18Meter.FillSTKReq(ID, Index: Integer);
begin
   m_QFNC       := $06;
   m_nTxMsg.m_sbyInfo[0] := $7E;
   ...
   FPUT(BOX_L1, @m_nTxMsg);
   m_nRepTimer.OnTimer(m_nP.m_swRepTime);
   TraceM(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer18::>Out CMD:',@m_nTxMsg);
end;

procedure CE9STKVer18Meter.CreateSpisSumEnReq(var nReq: CQueryPrimitive);
begin
   {
...
procedure CE9STKVer18Meter.CreateSpisnnNakEnMonReq(var nReq: CQueryPrimitive);
var
    pMsg: CMessage;
begin
    if (nReq.m_swSpecc0=0) then ReadNakEnMon(pMsg)
    else
    Begin
     nReq.m_swParamID := QRY_NAK_EN_MONTH_EP;
     m_ParamID := QRY_NAK_EN_MONTH_EP;
     FillSTKReq(32,nReq.m_swSpecc0-1);
     TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer49::>Чтение нач мес по адресу: ' + IntToStr(nReq.m_swSpecc0));
    End;
end;

procedure CE9STKVer18Meter.CreateSpisnnNakEnDayReq(var nReq: CQueryPrimitive);
begin
   if nReq.m_swSpecc1 = 0 then
   begin
     FinalAction;
...
procedure CE9STKVer18Meter.CreateSumEnergyReq(var nReq: CQueryPrimitive);
begin
   FillSTKReq(12, 0);
end;

procedure CE9STKVer18Meter.CreateSumEnReq(var nReq: CQueryPrimitive);
begin
   m_nTxMsg.m_sbyInfo[0] := $7E;
   m_nTxMsg.m_sbyInfo[1] := $00;
...
procedure CE9STKVer18Meter.ReadMonthEnAns(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   for i:=0 to 3 do
   Begin
    SwapDW(pVal.ENERGY[i]);
    if i=0 then
    Begin
     FillSaveDataMessage(0, QRY_ENERGY_MON_EP, 0, tempDate);
     FPUT(BOX_L3, @m_nRxMsg);
    End;
    FillSaveDataMessage(pVal.ENERGY[i]*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff, QRY_ENERGY_MON_EP, i+1, tempDate); FPUT(BOX_L3, @m_nRxMsg);
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>VMON:'+
     ' DT :'+ DateTimeToStr(tempDate)+
     ' Tar:'+ IntToStr(i)+
     ' E+ :'+ FloatTostrF(pVal.ENERGY[i]*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff,ffFixed,6,4));
   end;
   FinalAction;
end;
{
...
{
procedure CE9STKVer18Meter.GetCurrentSID(var nSID:Word;var dtDate:TDateTime);
Var
    m_SresID : Integer;
    m_QTimestamp : TDateTime;
Begin
...
begin
   ...
   FillSaveDataMessage(dbEnergyPok, QRY_NAK_EN_DAY_EP, 1, dtDate01);
   FPUT(BOX_L3, @m_nRxMsg);
   OnFinalAction();
End;
procedure CE9STKVer18Meter.CreateDayEnAns(var nReq: CQueryPrimitive);
var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,nINdex :Integer;
...
begin
   ...
   if m_bData.FGetSum(dtDate01,pData)=true then
    ...
    FillSaveDataMessage(pData.dbEnergy, QRY_ENERGY_DAY_EP, 1, dtDate01+1);
    //FillSaveDataMessage(pData.dbEnergy{*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff}, QRY_ENERGY_DAY_EP, 1, dtDate01+1);
    FPUT(BOX_L3, @m_nRxMsg);
    End;
    OnFinalAction();
End;
procedure CE9STKVer18Meter.Set30Buffer(wShift:Word);
Var
    pData : CStk43Srez;
    dtDate,dtDate00,dtDate01 : TDateTime;
    i,j,nINdex :Integer;
Begin
  ...
  //End;
End;
procedure CE9STKVer18Meter.ProcessMonthReq(var nReq: CQueryPrimitive);
...
begin
   ...
   FillSTKReq(32,nReq.m_swSpecc0);
   TraceL(2, m_nTxMsg.m_swObjID,'(__)CE9STKVer18::>Чтение месячных архивов по адресу: ' + IntToStr(nReq.m_swSpecc0));
end;


function CE9STKVer18Meter.IsDateTimeEqual(dt1, dt2 : TDateTime): boolean;
...
function CE9STKVer18Meter.GetULongDate(time: Cardinal): TDateTime;
...
function CE9STKVer18Meter.GetUIntDate(time: word): TDateTime;
var yy, mm, dd, hh, nn, ss : word;
begin
   ss := 00;
   nn := (time and $03) * 15;
...
function CE9STKVer18Meter.GetDTFromSM(sm: integer): TDateTime;
...
procedure CE9STKVer18Meter.ProcessSliceReq(var nReq: CQueryPrimitive);
begin
   CreateSliceReq(nReq);
end;
{
...
function  CE9STKVer18Meter.GetNextPointer(nIndex : integer): integer;
Begin
   if (m_pPoint.LASTuk-nIndex)>=0 then Result := m_pPoint.LASTuk-nIndex;
   if (m_pPoint.LASTuk-nIndex)<0  then Result := 8999+(m_pPoint.LASTuk-nIndex);
End;

procedure CE9STKVer18Meter.CreateSliceReq(var nReq: CQueryPrimitive);
Var
   pData : CStk43Srez;
...
begin
   ...
   case m_SliceReadSt of
     ...
     STSL_STKV116_READ_SLICE  : begin
                                 ...
                                 Month := HiByte(nReq.m_swSpecc1);
                                 Day   := LoByte(nReq.m_swSpecc1);
                                 ...
                                 //if m_bData.FINDStk43(dtDate0,pData)=True then
                                 Begin
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ06_GetDataByID(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $06;
  m_QTimestamp := Now();
...
function CE9STKVer18Meter.SwapDW(var swVal:Dword):Dword;
var
...
Begin
    ...
    Result := swVal;
End;
procedure CE9STKVer18Meter.ReadSpisEnerAns(var pMsg: CMessage);
var
    pVal : _poket;
    i : Integer;
begin
   ...
   //IsTrueValue(pVal.total);
   if (nReq.m_swSpecc1=0) then
   Begin
    //for i:=0 to 7 do m_dbNakEn[i] := 0;
    m_dbNakEn[0]    := 0;
    m_dbEnMonTek[0] := 0;
    FillSaveDataMessage(0, QRY_ENERGY_SUM_EP, 0, Now);FPUT(BOX_L3, @m_nRxMsg);
    //TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>VSpis:'+
    //' DT :' + DateTimeToStr(Now)+
    //' Tar:' + IntToStr(nReq.m_swSpecc1)+
    //' Es+:' + FloatTostrF(m_dbNakEn[0],ffFixed,6,4)+
    //' E+ :' + FloatTostrF(m_dbNakEn[nReq.m_swSpecc1+1],ffFixed,6,4) );
   End;
   ...
   Begin
    ...
    //m_dbNakEn[0]    := m_dbNakEn[0]    + pVal.total*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff;
    //m_dbEnMonTek[0] := m_dbEnMonTek[0] + pVal.curr_month*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff;
   End;
   //TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>VSpis:'+
   //' DT :' + DateTimeToStr(Now)+
   //' Tar:' + IntToStr(nReq.m_swSpecc1)+
...
procedure CE9STKVer18Meter.ReadCurrentParam(var pMsg: CMessage);
var
   pVal : DWORD;
   dbValue : Double;
begin
   ...
   SwapDW(pVal);
   dbValue := pVal*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff/1000;
   FillSaveDataMessage(dbValue, QRY_MGAKT_POW_A, 0, Now);FPUT(BOX_L3, @m_nRxMsg);
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>VCurrPR:'+
   ' PA:' + FloatTostrF(pVal,ffFixed,6,3));
   FinalAction;
end;
{
nReq.m_swSpecc0
}
procedure CE9STKVer18Meter.ReadNakEnMon(var pMsg: CMessage);
var
    tempDate : TDateTime;
    i     : integer;
    value : double;
...
begin
   ...
   DecodeDate(Now,Year1,Month1,Day1);
   tempDate := EncodeDate(Year1,Month1,1);
   if (nReq.m_swSpecc0=0) then for i:=0 to 7 do m_dbEnMon[i] := 0;
   cDateTimeR.DecMonthEx(nReq.m_swSpecc0,tempDate);
   for i:=0 to 3 do
   Begin
    SwapDW(pVal.ENERGY[i]);
    if (nReq.m_swSpecc0=0)  then m_dbEnMon[i+1] := m_dbEnMon[i+1] + m_dbEnMonTek[i+1] else
    if (nReq.m_swSpecc0<>0) then m_dbEnMon[i+1] := m_dbEnMon[i+1] + pVal.ENERGY[i]*m_nP.m_sfKI*m_nP.m_sfKU*m_nP.m_sfMeterKoeff;
    if i=0 then
    Begin
     FillSaveDataMessage(0, QRY_NAK_EN_MONTH_EP, 0, tempDate);
     FPUT(BOX_L3, @m_nRxMsg);
    End;
    FillSaveDataMessage(m_dbNakEn[i+1]-m_dbEnMon[i+1], QRY_NAK_EN_MONTH_EP, i+1, tempDate); FPUT(BOX_L3, @m_nRxMsg);
    TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>VNMON:'+
    ' DT :'+ DateTimeToStr(tempDate)+
    ' Tar:'+ IntToStr(i)+
    ' E+ :'+ FloatTostrF(m_dbNakEn[i+1]-m_dbEnMon[i+1],ffFixed,6,4));
   End;
...
procedure CE9STKVer18Meter.ReadSlicePeriodAns(var pMsg: CMessage);
var perMask : integer;
begin
...
procedure CE9STKVer18Meter.ReadPointSliceAns(var pMsg: CMessage);
...
begin
{
 LASTnum    : Word;            	// Указатель на ячейку LAST
   ...
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>POINT_0:'+
    ' LASTnum :'+ IntToStr(m_pPoint.LASTnum)+
    ' LASTuk :'+ IntToStr(m_pPoint.LASTuk));
   m_pPoint.LASTnum := Swap(m_pPoint.LASTnum);
   m_pPoint.LASTuk  := Swap(m_pPoint.LASTuk);
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>POINT_1:'+
    ' LASTnum :'+ IntToStr(m_pPoint.LASTnum)+
    ' LASTuk :'+ IntToStr(m_pPoint.LASTuk));
...
procedure CE9STKVer18Meter.ReadSliceAns(var pMsg: CMessage);
var
...
begin
   ...
   FinalAction;
   {
   TraceL(3, m_nTxMsg.m_swObjID, '('+IntToStr(m_nIndex)+')CE9STKVer18::>Srez:'+
...
procedure CE9STKVer18Meter.SendSliceToL3(dValue:Double;wType:Word;dtDate:TDateTime);
begin
...
procedure CE9STKVer18Meter.SendSliceToByIndexL3(dValue:Double;wType:Word;nIndex:Integer;dtDate:TDateTime);
begin
...
procedure CE9STKVer18Meter.SetUnknownSlice(wType:Word);
var tempDate : TDateTime;
begin
...
procedure CE9STKVer18Meter.SetUnknownSliceByIndex(wType:Word;nIndex:Integer);
var tempDate : TDateTime;
begin
   tempDate := GetDTFromSM(nReq.m_swSpecc0);
   FillSaveDataMessage(0, wType, 0, tempDate);
...
procedure CE9STKVer18Meter.ReadSlicesPackAns(var pMsg: CMessage);
begin
...
procedure CE9STKVer18Meter.ReadJrnlAns(var pMsg: CMessage);
var i, uk : integer;
    tDate : TDateTime;
    time  : Cardinal;
begin
 ...
 * @param var pMsg : CMessage
 *******************************************************************************)
procedure CE9STKVer18Meter.RES01_GetDataByID_Fail(var pMsg : CMessage);
var
  l_EStr : String;
begin
  ...
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::>' + l_EStr);
  FinalAction();
end;
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ07_GetDateTime(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $07;
  m_QTimestamp := Now();
...
procedure CE9STKVer18Meter.RES07_GetDateTime(var pMsg:CMessage);
var
  nReq : CQueryPrimitive;
...
procedure CE9STKVer18Meter.KorrTime(LastDate : TDateTime);
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer18Meter.RES00_SetDateTime_OK(var pMsg:CMessage);
begin
  if pMsg.m_sbyInfo[2]=0 then
  Begin
   TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Время счетчика установлено: ' + DateTimeToStr(Now()));
...
procedure CE9STKVer18Meter.CreateJrnlEvReq(var nReq: CQueryPrimitive);
begin
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ0F_GetCurrentPower(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0f;
  m_QTimestamp := Now();
 ...
 * @param var nReq: CQueryPrimitive Примитив запроса
 ******************************************************************************)
procedure CE9STKVer18Meter.REQ0E_CTRL_SetRelayState(var nReq: CQueryPrimitive);
begin
  m_QFNC       := $0E;
  m_QTimestamp := Now();
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer18Meter.RES00_CTRL_SetRelayState_OK(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Операция с контактором выполнена успешно');
  FinalAction();
end;
 ...
 * @param var pMsg:CMessage
 *******************************************************************************)
procedure CE9STKVer18Meter.RES01_CTRL_SetRelayState_Fail(var pMsg:CMessage);
begin
  TraceL(2, m_nTxMsg.m_swObjID, '(__)CE9STKVer18::> Операция с контактором НЕ ВЫПОЛНЕНА!');
  FinalAction();
end;
 ...
 ******************************************************************************)
procedure CE9STKVer18Meter.ADD_RelayState_CTRLQry(_StateID : WORD);
begin
 ...
 ******************************************************************************}
procedure CE9STKVer18Meter.ADD_Events_GraphQry();
begin
  m_nObserver.ClearGraphQry();
 ...
 * Формирование задания на считывание события
 ******************************************************************************}
procedure CE9STKVer18Meter.ADD_DateTime_Qry();
begin
  m_nObserver.ClearGraphQry();
...
procedure CE9STKVer18Meter.ADD_Energy_Mon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
procedure CE9STKVer18Meter.ADD_Energy_NMon_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
    ...
    m_nObserver.AddGraphParam(QRY_ENERGY_SUM_EP, 0, 4, 0, 1);
   if (cDateTimeR.CompareMonth(dt_Date2, Now) = 1 ) then
     dt_Date2 := Now;
   while cDateTimeR.CompareMonth(dt_Date1, dt_Date2) <> 1 do
   begin
 ...
 ******************************************************************************}
procedure CE9STKVer18Meter.AddSresEnergGrpahQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i, Srez     : integer;
    h, m, s, ms : word;
    y, d, mn    : word;
...
procedure CE9STKVer18Meter.ADD_Energy_Day_GraphQry(dt_Date1, dt_Date2 : TDateTime);
var TempDate    : TDateTime;
    i           : integer;
begin
    //m_nObserver.AddGraphParam(QRY_AUTORIZATION, 0, 0, 0, 1);
...
function CE9STKVer18Meter.GetYear(month, day: integer): integer;
var yN, mN, dN : word;
    tempDate   : TDateTime;
begin
...
function CE9STKVer18Meter.IsTrueValue(var dbVal:Double):Boolean;
Begin
   Result := True;
