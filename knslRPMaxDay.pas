...
begin
   ...
   if ParName = 'Pmax5'        then ParValue := 'Максимальная мощность за день (' + strEK[KindEnergy] + ')';
end;
...
begin
   ...
   if IsRPGroup = 0 then
   Begin
    globalTtlMainName := 'Совмещенные максимумы мощности в ' + cDateTimeR.GetNameMonth(Month)+ ' ' + IntToStr(Year) + ' года' + ' в часы максимальных нагрузок энергосистемы ';
    globalTtlNameTbl  := 'Группа учета: ' + FsgGrid.Cells[1, ItemInd]
   End else
   Begin
    globalTtlMainName := 'Получасовые максимумы мощности в ' + cDateTimeR.GetNameMonth(Month)+ ' ' + IntToStr(Year) + ' года' + ' в часы максимальных нагрузок энергосистемы ';
    globalTtlNameTbl  := 'Точка учета: ' + FsgGrid.Cells[1, ItemInd];
   End;
   ...
   globalTtlMainName := globalTtlMainName + DateOutSec(m_pTariffs.Items[2].m_dtTime0) + ' - ' + DateOutSec(m_pTariffs.Items[2].m_dtTime1) +
                         ' и ' +DateOutSec(m_pTariffs.Items[1].m_dtTime0) + ' - ' + DateOutSec(m_pTariffs.Items[1].m_dtTime1) else
