...
Begin
    ...
    Begin
     ...
     begin
      ...
      begin
       if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'('+IntToStr(getPullID)+','+IntToStr(connection.IsParam)+')CPortPull захвачен порт getConnection('+connection.getConfig.CONNSTRING+') ... ');
...
Begin
   try
    ...
    csBusy.Leave;
     ...
     if EventBox<>Nil then EventBox.FixEvents(ET_Critical,'('+IntToStr(287)+' строка)Connection.OnTimeElapsed!!!');
   end;
End;
...
//      if port=nil then
//        begin
//          if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'(Error GsmConnection.connect!!! Не существует связи Gsm порта');
//          exit;
//        end;
     port := mL1Module.m_pPort[index];
    ...
    // sleep(1000);     
     ...
     End;
      ...
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')Ошибка в GsmConnection.connet ');
 end;
End;
  ...
  try
   try
     ...
     if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')GsmConnection.close DISCONNECT Complette... ');
      ...
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')Ошибка в GsmConnection.close ');
   end;
  finally
//    if (mL1Module.m_pPort[index]<>nil)then
//           mL1Module.DelNodeLv(Index);
...
//     if port=nil then
//      begin
//        if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'(Error ComConnection.connect!!! Не существует связи com порта');
//        exit;
//      end;
//     sleep(1000);
     port  := mL1Module.m_pPort[index];
     ...
     End;
      ...
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')Ошибка в ComConnection.connect ');
 end;
End;
...
{     if assigned(mL1Module) then
     ...
     if port=nil then
      begin
       if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'(Error TcpConnection.connect!!! Не существует TCP порта');
       exit;
      end;
//     sleep(1000);      
     port.Connect(pMsg);
     ...
     End;
      ...
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')Ошибка в GsmConnection.connect ');
 end;
End;
 ...
 try
   try
     ...
     if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')TcpConnection.close DISCONNECT Complette... ');
      ...
      if EventBox<>Nil then EventBox.FixEvents(ET_NORMAL,'('+IntToStr(param.PULLID)+')Ошибка в TcpConnection.close ');
   end;
 finally
//    if (mL1Module.m_pPort[index]<>nil)then
//           mL1Module.DelNodeLv(Index);
