...
begin
  ...
  m_nTxMsg.m_sbyInfo[3]     := m_nRtr or (m_nRtr shl 4);  // :=0; // резерв
...
begin
  ...
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Устройство в сети. Протокол: ' + IntToStr(l_ProtoVer) +
...
begin
  m_QTimestamp := Now;
  //nSH := 2*m_nRtr;
  TraceL (2, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Устройство в сети. Протокол:RES05_GetEnergySum ');
...
begin
    ...
    if byStatus=$00 then
    Begin
     TraceL(3, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Время счетчика установлено: ' + DateTimeToStr(Now()));
    ...
    End else
    Begin
     if byStatus=$01 then  TraceL(3, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Попытка записи времени с неверным паролем ') else
     if byStatus=$05 then  TraceL(3, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Интерфейс заблокирован ') else
     TraceL(3, m_nTxMsg.m_swObjID, '(__)CMIRW1::> Ошибка:'+IntToStr(byStatus));
     ErrorCorrEv;
    End;
    FinalAction();
    m_QFNC:=0;
...
begin
 ...
 l_tValueChastota :=0;
 l_tValueCosinus  :=0;
 move(pMsg.m_sbyInfo[18+nRSH], l_tValueActiv, 2); // достаем активную мощность 2 Byte
 move(pMsg.m_sbyInfo[20+nRSH], l_tValueReactiv, 2); // достаем реактивную мощность 2 Byte
 move(pMsg.m_sbyInfo[22+nRSH], l_tValueChastota, 2); // достаем cos 2 Byte
 move(pMsg.m_sbyInfo[24+nRSH],l_tValueCosinus, 2); // достаем частоту 2 Byte
