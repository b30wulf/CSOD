...
begin
   ...
   begin
     ...
     begin
       FillChar(m_nTxMsg.m_sbyInfo[6 + i*18], 18, 0);  //Проверка выхода за пределы структуры
       exit;
     end;
...
begin
   ...
   begin
     ...
     begin
       FillChar(m_nTxMsg.m_sbyInfo[6 + i*64], 64, 0);  //Проверка выхода за пределы структуры
       exit;
     end;
...
begin
   ...
   begin
     ...
     begin
       FillChar(m_nTxMsg.m_sbyInfo[6 + i*52], 52, 0);  //Проверка выхода за пределы структуры
       exit;
     end;
     Temp  := i + NT;
...
begin
   ...
   begin
     ...
     begin
       exit;
       FillChar(m_nTxMsg.m_sbyInfo[LengthMSG], 52, 0);  //Проверка выхода за пределы структуры
...
begin
   ...
   if (Ti = 0) then
     if (pMsg.m_sbyInfo[1] and $80 = 0) or (pMsg.m_sbyInfo[pMsg.m_swLen - 11 - 3] <> 12) then
       NT := 1;     //Не учитываем количество тарифов, если пакет не наш
...
begin
   ...
   if (wMeterNum = wTemp) and (abs(dwLastTime - timeGetTime) < byTimeTranz) then //Время транзакции не закончено
     Kode := 9;
   wMeterNum   :=  wTemp;
   ...
   byTimeTranz := pMsg.m_sbyInfo[9];                                             //Время транзакции
...
begin
   ...
   if FindPortID(wMeterNum) = High(word) then             //Если порта не сушествует закр. транз.
     exit;
   StartTranz301;
   if EventBox<>Nil then EventBox.FixEvents(ET_CRITICAL,'(__)CL4MD::>BTI CreateCC301TranzReq 0');
...
procedure CBTIModule.SendMsgToMeter(var pMsg : CMessage); //Отправка сообщения напрямую в счетчик
begin
   if FindPortID(wMeterNum) = High(word) then             //Если порта не сушествует закр. транз.
     exit;
   StartTranz;
...
Begin
    ...
    if (byTimeTranz > abs(timeGetTime - dwLastTime)/1000) then //Транзакция открыта ???
    begin
      dwLastTime := timeGetTime;
    ...
    case fnc of     
      ...
      $00A2 : FNCMaxDay(pMsg);
      $FF00 : FNCStartPool(pMsg);                            //Запустить сервер
      $FF01 : FNCStopPool(pMsg);                             //Остановить сервер
      $FF02 : FNCReadGraphFromMeters(pMsg);                  //Прочитать графики/архивы из устройств
      $FF03 : FNCReBoot(pMsg);                               //Перезагрузка
      $FF04 : FNCReadEvents(pMsg);                           //Запрс событий
      $FF05 : FNCStopL3(pMsg);                               //Останов L3
      $FF06 : FNCStartFH(pMsg);                              //Останов L3
      $FF07 : FNCSetL2TM(pMsg);                              //Установка ограничения коррекции времени
      ...
      $FF12 : FNCSetSynchro(pMsg);                            //Установка источника синхронизации
      $FF13 : FNCDeepBuffer(pMsg);                            //Установка глубины буфера параметров сети
      $FF14 : FNCBaseSize(pMsg);                              //Установка максимального размера базы
      $FF15 : FNCDeepData(pMsg);                              //Установка глубины хранения данных
      $FF16 : FNCClearData(pMsg);                             //Удаление  данных
      $FF17 : FNCExecSQL(pMsg);                               //Выполнение запроса
      $FF18 : FNCInit(pMsg);                                  //Инициализация
      $FF19 : FNCDelArch(pMsg);                               //Удаление архивов
      $FF1A : FNCReadAllCurrParams(pMsg);                     //Чтение всех текущих параметров одним запросом
